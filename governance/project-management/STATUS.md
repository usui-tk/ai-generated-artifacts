...
_Last updated: 2026-06-02 (UTC) — **ADR 0017 (structure-first review methodology, a→b) finalized**. The review-order methodology used throughout — verify structural validity inside the normative set FIRST (a), investigate the current state / current artifacts SECOND (b); on each finding re-scan the governance md/ADRs + full forward/back impact analysis + split now-decisions from later-deferrable — is promoted from baseline prose to an in-repo decision (SPEC §execution-framework), the epistemic-order complement to ADR 0004. Deliberately finalized AFTER P3 ran a full cycle so its benefits ((a) found F1/F3/F4 from documents alone; re-scan reclassified/corrected findings) AND limits ((b) was unavoidable for region_locator / `unknown` conditions / the `bash` profile / the whole-tool sentinel convention) are evidenced, not assumed. No code/canon/schema change. **Next: P3a** (manifest CRUD tool, ADR 0011). **[P4.4 decision owed]:** promote the whole-tool sentinel-value registration convention (AGENTS.md §8) to an ADR after the psa.py 2nd instance. Prior: P3.7 (P3 phase closed); P3.6 scanner build; ADR 0016 / P3.1; ADR 0015 / P3.0._
...
| Field | Value |
|:---|:---|
| Just completed | **ADR 0017 — structure-first review methodology (a→b)**: promoted the review-order methodology (structural-validity-first / current artifacts-second; re-scan + impact analysis + now-vs-later split on each finding) from baseline prose to an in-repo decision (SPEC §execution-framework), the epistemic complement to ADR 0004. Finalized after P3's full cycle (evidence both ways). No code/canon/schema change. *(Prior: **P3.7** P3 closed; **P3.6** scanner build; **ADR 0016 / P3.1**; **ADR 0015 / P3.0**.)* |
...
— see the P6-investigation notes in `scanner.py`) and the (b) consumer current-state/operational

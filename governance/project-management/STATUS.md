...
_Last updated: 2026-06-01 (UTC) — **P2a complete: the `reference-code/powershell` canon is RELEASED at 1.0.0 and is now vendorable** (ADR 0008 §4). The full 58-unit behavioral test suite was authored and wired into a single-entry gate (71 passed / 1 skipped / 0 failed); all 58 units promoted `0.1.0 → 1.0.0` with `tested=true`. Added **ADR 0010** (canon test taxonomy + data management), **ADR 0011** (canon change-management governance), **ADR 0012** (dual-runtime env-info policy), **ADR 0013** (multi-platform / multi-version QA). Next phase **P3** (drift scanner)._
...
| Field | Value |
|:---|:---|
| Just completed | **P2a — author + wire + release the canon** (P2a.1–.3b): canon test home (ADR 0010) → 58-unit behavioral suite (Canon.Unit/Env/Smoke + T11) → single-entry gate runner `tests/Invoke-CanonTests.ps1` → SemVer promotion `0.1.0 → 1.0.0` + `tested=true` for all 58. Plus **ADR 0011/0012/0013** governance/policy/QA. Canon **released, vendorable** (ADR 0008 §4). P2 itself closed at `867efe3` (manifest `maturity` deferred to TF) |
| Current phase | **P2a closed** — this commit (P2a.4) brings STATUS current; next phase **P3** (consumer-drift scanner: detect divergence between consumers and canon, feeding the ADR 0011 reconcile-back process) |
...
| **P2a.4** — close P2a (STATUS) | ✅ this commit | STATUS brought current to canon 1.0.0 / P2a complete; next phase P3 |

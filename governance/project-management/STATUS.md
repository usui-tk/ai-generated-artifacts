...
_Last updated: 2026-06-02 (UTC) — **ADR 0015 (canonical normalized-hash contract) accepted; P3 opened (P3.0 = hash-contract pin)**. Promotes baseline §4.5 into the in-repo SPEC as a **computable** contract (region body → strip comments/strings → collapse whitespace → `sha256`/**16-hex**); adds the governance-state-validator **read-side check G** (recompute marker hash + verify marker `policy`/`binding` vs manifest) and a **write-side `canon-hash-restamp` tool**; pins conformance by **golden vectors** (no shared import). Check G surfaced a latent defect: **20 of 58 markers were mis-stamped at canon creation (`5d5f0b1`)** — corrected by the re-stamp tool, **metadata-only, no version bump** (bodies byte-identical). Interim **metadata guardrail**: marker/manifest edits must pass the validator (incl. G) at the §Y dry-run before the patch is cut, bridging to the ADR 0011 CRUD tool (P3a). Prior: ADR 0014; P2a complete (canon released 1.0.0)._
...
| Field | Value |
|:---|:---|
| Just completed | **ADR 0015 — canonical normalized-hash contract** + **P3.0 (P3 opening: hash-contract pin)**: promoted baseline §4.5 into the in-repo SPEC as a computable contract (strip comments/strings → collapse whitespace → `sha256`/16-hex); added validator **check G** (recompute marker hash) + extended **check D** (marker `policy`/`binding` vs manifest); added the **`canon-hash-restamp`** write-side tool; pinned conformance with **golden vectors GV-1..5**. Check G caught **20/58 markers mis-stamped at `5d5f0b1`**; re-stamped **metadata-only, no version bump**. Self-test **7→15**. *(Prior: **ADR 0014** document governance; **P2a** — canon released 1.0.0.)* |
| Current phase | **In P3; P3.0 (hash-contract pin) complete** — the drift contract is now pinned and verified, so the scanner has a real reference. Next: **P3 main** (consumer-drift scanner: detect divergence between the consumer scripts and the canon, feeding the ADR 0011 reconcile-back process), then **P3a** (manifest CRUD tool — after which direct manifest/marker edits stop and the interim guardrail is superseded) |
...
## Next action
**P3.0 (hash-contract pin) is complete** — the drift contract is pinned (ADR 0015) and
verified (check G; 20 mis-stamped markers corrected). Proceed to **P3 main** — the
consumer-drift scanner (`quality-tools/canonical-drift-scanner/`): detect where the 6 consumer
scripts have diverged from the canon, reuse-by-copying the ADR 0015 normalizer (passes
GV-1..5), emitting `observation.schema.json` records, feeding the **ADR 0011** reconcile-back
process (code → quality gates → manifest CRUD → commit + CHANGELOG). **P3a** then builds the
manifest CRUD tool (`quality-tools/canon-manifest-tool/`) + machine trigger — after which
direct manifest/marker edits stop and the **interim metadata guardrail (ADR 0015 §6) is
superseded**. Vendoring (P6/P7) is UNBLOCKED (canon ≥ 1.0.0) but later. Remediate `$ks` at **P6**.

...
<!-- AI read-contract: authoritative for the REVIEW-ORDER methodology used when a new
     finding or design question appears - (a) verify structural validity inside the
     normative set FIRST, (b) investigate the current state / current artifacts SECOND, and on every
     new finding re-scan the governance md/ADRs, run a full forward/back impact analysis,
     and split now-decisions from items deferrable until implementation/the artifacts are visible.
...
# 0017 — Structure-first review methodology (a→b)
...
single largest risk is **letting the current artifacts drive the model** — if a
...
1. **The model is defined across many Markdown files + ADRs** (baseline, plan, spine,
   ...
   current artifact.
2. **Some defects only appear when the design meets the current artifacts** — an inlined value with no
...
Neither "documents only" nor "artifacts only" is sufficient; the **order** matters.

## Decision

Adopt a fixed review order for every new finding or design question:

1. **(a) Structural validity in the normative set, FIRST.** Before inspecting any current
...
2. **(b) Current-state (artifacts) investigation, SECOND.** Only after the structure is settled
   do we inspect real artifacts/operations — to *fill* the design (pin values, validate
   applicability, exercise real failure modes), never to *decide* the design. The artifacts fill
...
- **split now-decisions from later-deferrable** ones, and for each deferred item name
  **what** must be decided and **how** it will be investigated when the artifacts are visible.

**Structural correctness sits above the current artifacts.** When the two appear to conflict, the
...
- **(a) found defects from documents alone:** P3.1 pinned three observation-schema fields
...
- **(b) was genuinely necessary — items (a) could not close:** at design time, three P6
...
This dual evidence — (a) closing most of the structure, (b) being unavoidable for a named
remainder — is exactly why the **order** (not one or the other) is the decision.

## Alternatives considered

- **current artifacts-first (inspect artifacts, then design):** rejected — it is the project's main risk
  made into a method; the design would inherit the artifacts' accidents (the P3.0 20-marker
  mis-stamp is a concrete example of an artifact that, if trusted, would have corrupted the
  contract).
- **Documents-only (never investigate the artifacts):** rejected — it cannot find applicability

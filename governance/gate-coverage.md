...
| 6 | canon-manifest-tool self-test | `python3 quality-tools/canon-manifest-tool/test_tool.py` | 19 cases: transactional CRUD incl. rollback-on-finding, kind=project lifecycle rows, marker-coupled writes REFUSED (the write-path boundary until the G4 coupled op) |
| 7 | canon-drift-trigger self-test | `python3 quality-tools/canon-drift-trigger/test_trigger.py` | 23 cases: observation→change-request derivation |

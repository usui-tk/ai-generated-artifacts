...
* **Red Hat のサブスクリプション規約**（特に entitled コンテンツへのアクセスや
...
* **entitled モード**は、サブスクリプション登録済みホストにおける podman 自身の
  自動注入（`/run/secrets`）に依拠すること。スイートは何もマウントせず、シーク
  レットを一切扱わない — コンテナは素のまま実行され、メジャーごとの entitled
  リポジトリが現れるかどうかはホスト側の働きによる。
...
コンテナに提供しているかどうかによって変わります。本スイートはこの問いを
...
1. Red Hat の UBI イメージは**匿名で**（トークン取得ステップなしで）プルでき、
   ...
   `rhel6/rhel` ベースを使う。
2. **サブスクリプション登録済みの RHEL ホスト**（rootful podman）上では、ホストが
   すべてのコンテナへ entitlement を**自動注入**する（`/run/secrets`。マウント
...
bash-rhel-container-testsuite/
  ...
  tests/
    ...
    collect-aws-rhui-facts.sh      # AWS RHUI 情報の単体収集ツール -> tar.gz（r73; v1.8.0
...
アウトバウンド通信が可能なホストでスイート全体の健全性を素早く確認するには、
1 コマンドで「全メジャー × 各ツール 1 サンプル」を実行して判定表を表示する
スモークが使えます（テスト用にプロビジョニングしたイメージは終了時に削除されます）。
...
* ✅ **Phase 0 — 実現可能性調査** — 実測した基礎事実（メジャー別 glibc、匿名リポジトリ
  集合、匿名プル、5 メジャー全体の entitled アクセス、RHEL 7 の固定タグ署名、EPEL
  エンドポイント）。Phase 0 で実測。
...
* ✅ **Phase 7 — 一般化** — フレームワークが**ツール非依存**になりました:
  ...
  **スイート緑: 22 階層、546 件成功、0 件失敗**(r36; ShellCheck はデフォルト重大度と `-S style` の両方でクリーン、CWD 非依存)。

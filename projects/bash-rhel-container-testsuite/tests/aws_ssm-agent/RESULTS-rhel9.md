...
On RHEL 9, the newest agent (3.3.4793.0) is **compliant-capable**. Cells are filled from the ledger by the run (`ran/installed`); `pending` = not yet run. Agents below 3.3.3598.0 are out of scope (ec2messages-only).

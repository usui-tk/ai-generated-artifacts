...
On RHEL 8, the newest agent (3.3.4793.0) is **compliant-capable**. Cells are filled from the ledger by the run (`ran/installed`); `pending` = not yet run. Agents below 3.3.3598.0 are out of scope (ec2messages-only).
...
_Install is empirically gated by the RPM dep closure + glibc; 11 versions in scope. Regenerate this evidence with a single run: `OSMAJORS=8 ./run-ssm-installtest-matrix.sh`._

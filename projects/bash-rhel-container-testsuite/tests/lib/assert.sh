# ----- Purpose --------------------------------------------------------------
#   Test-harness library: assertion + tier-summary helpers (t_pass/t_fail/assert_*).
# ----- Prerequisites --------------------------------------------------------
...
#
# Ported verbatim from projects/bash-ol-aws-ami-builder/tests/lib/assert.sh
...
# load-dependent false negative. A here-string has no upstream writer process in
# the pipeline, so the command status is grep's alone and the race cannot occur.
# (`<<<` appends a trailing newline, which is harmless for these line-anchored
# EREs.)
assert_match() {
  if grep -Eq -- "$2" <<<"$1"; then
    t_pass "$3"

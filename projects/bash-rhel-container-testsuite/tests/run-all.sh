...
# one entry point that runs every host-runnable tier (tests/tNNN_*.sh), aggregates
...
#
# Scope note: this runner aggregates L0-L2 (static + hermetic-unit + component)
# tiers only. The L3 integration matrices (tests/aws_*/run-*-matrix.sh) perform
# real podman pulls and installs and are run manually / in CI, not from here -
# see TESTING.md "Execution model & test tiers" and the design plan sec 9.
#
...
echo "== bash-rhel-container-testsuite test suite =="
echo "  bash:       $(bash --version | head -1)"
if command -v shellcheck >/dev/null 2>&1; then
  echo "  shellcheck: $(shellcheck --version 2>/dev/null | awk '/^version:/ {print $2}')"
else
  echo "  shellcheck: (not installed - the L0 ShellCheck tier will SKIP)"
fi
if command -v podman >/dev/null 2>&1; then
  echo "  podman:     $(podman --version 2>/dev/null | awk '{print $3}') (L3 acquisition engine; not exercised by L0-L2)"
else
  echo "  podman:     (not installed - L3 integration matrices use the curl-only OCI fallback or SKIP)"
fi
echo

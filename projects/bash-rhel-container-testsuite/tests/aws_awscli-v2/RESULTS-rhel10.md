# AWS CLI v2 - RHEL 10 install/run results
...
AWS CLI v2 ships a self-contained zip bundle that BUNDLES its own Python, so it does not use the OS Python - but the bundled interpreter and its shared objects are built against a **manylinux glibc**, so the OS **glibc** gates whether the bundle installs and runs. Per AWS *Linux Support Updates for AWS CLI v2* (2024-09-16), current v2 is built on **manylinux2014 (glibc 2.17)** and supports glibc >= 2.17; systems on glibc <= 2.16 should pin v2 **<= 2.17.49**. This report characterizes, per RHEL 10 environment, which v2 versions install + run - i.e. the newest AWS CLI v2 a RHEL 10 image can actually use.
...
On RHEL 10 (glibc 2.39): current versions (>= 2.17.50) are **runs**; the legacy band (<= 2.17.49) **runs** (floor 2.5).
...
|:--|:--|:--|:--|:--|:--|:--|
| 2.0.0 | manylinux1 | 2.5 | 2.39 | runs | pending | pending |
| 2.17.49 | manylinux1 | 2.5 | 2.39 | runs | pending | pending |
| 2.17.50 | manylinux2014 | 2.17 | 2.39 | runs | pending | pending |
| 2.27.0 | manylinux2014 | 2.17 | 2.39 | runs | pending | pending |
...
- GA **2025-05-20**; Full Support through ~**2030-05-31**; Maintenance Support through ~**2035-05-31**.

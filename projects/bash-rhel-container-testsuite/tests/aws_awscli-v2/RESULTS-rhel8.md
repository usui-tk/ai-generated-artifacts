...
AWS CLI v2 ships a self-contained zip bundle that BUNDLES its own Python, so it does not use the OS Python - but the bundled interpreter and its shared objects are built against a **manylinux glibc**, so the OS **glibc** gates whether the bundle installs and runs. Per AWS *Linux Support Updates for AWS CLI v2* (2024-09-16), current v2 is built on **manylinux2014 (glibc 2.17)** and supports glibc >= 2.17; systems on glibc <= 2.16 should pin v2 **<= 2.17.49**. This report characterizes, per RHEL 8 environment, which v2 versions install + run - i.e. the newest AWS CLI v2 a RHEL 8 image can actually use.
...
- Full Support ended **2024-05-31**; Maintenance Support through **2029-05-31** (last minor 8.10).

#!/usr/bin/env bash
# ----- Purpose --------------------------------------------------------------
#   L1 unit: per-OS-major version pins in the root
# ----- Prerequisites --------------------------------------------------------
#   bash 4+; hermetic (no network); shellcheck only for t002.
# ----- Usage examples -------------------------------------------------------
#   bash tests/t015_installpins.sh
...
# --- ENA: RHEL 6 E2E-verified 2.9.1 (max in-scope on kernel 2.6.32); 7-10 current ---
assert_eq "2.9.1"  "$(resolve_pin "${ENA}" ENA_LIB_ONLY ENA_VERSION 6)"  "ena RHEL6 -> 2.9.1 (E2E verified, kernel 2.6.32)"

...
| `env.properties.aws-ol5` | Build parameters for **Oracle Linux 5 Update 11** (x86_64) — **deepest legacy / not shipped upstream / E2E-unproven**. Full host-supply model (no in-guest network on EL5); the operator-measured SHA256 `ISO_CHECKSUM` is recorded in the template (2026-07-19; mirror-MD5 cross-checked — the media is terminal, so the value is permanent). See sections 9.8 and 10 for important caveats. |
...
- **E2E status.** The synthesized shapes are gate-checked (`t025`, P3GATE OL5 branch) but **real EL5 anaconda contact and real EC2 boot are pending**; the recommended first contact is `SERIAL_CONSOLE="yes"` + `--build-only` to watch the install live. Nitro boot rests on the measured in-box `nvme` in UEK R2 plus the self-built ENA; Xen-generation instances are the measured fallback.
...
9. **Oracle Linux 5 specific limitations.**
   ...
   - **E2E-unproven.** Unlike OL6-10 (real-boot verified 2026-07-13), the OL5 path has **not yet had real EL5 anaconda contact or a real EC2 boot**. The kickstart shape, the virt-customize rpm transaction, and Nitro boot are all first-contact risks; use `SERIAL_CONSOLE="yes"` + `--build-only` for the first attempt.

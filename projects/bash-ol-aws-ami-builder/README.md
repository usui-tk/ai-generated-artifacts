...
| `env.properties.aws-ol5` | Build parameters for **Oracle Linux 5 Update 11** (x86_64) — **deepest legacy / not shipped upstream / E2E-unproven**. Full host-supply model (no in-guest network on EL5); requires a one-time operator-computed SHA256 `ISO_CHECKSUM` (the mirror MD5 is recorded in the template as the cross-check). See sections 9.8 and 10 for important caveats. |
...
- **ISO + checksum contract.** The 5.11 media lives on the kernel.org mirror under the original "Enterprise Linux" naming (`Enterprise-R5-U11-Server-x86_64-dvd.iso`). Upstream accepts SHA1/SHA256 checksums only while the mirror publishes MD5SUMS, so the operator computes the SHA256 once (cross-checking the mirror MD5 recorded in the env template) and bakes it into `ISO_CHECKSUM` — the build fails fast when it is empty.

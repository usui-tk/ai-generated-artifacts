...
| `install-ena-driver.sh` | Self-contained script that builds and installs a pinned (OL6 → 2.9.1, OL7 → 2.17.0) or latest-resolved (OL8/9/10 — see SPEC B.9) Amazon ENA driver via DKMS, and states the ENA Express readiness of the installed module. Installs its own deps (EPEL, gcc/make, dkms, `kernel-uek-devel`) and can be run **standalone on a stock OL instance** for validation, or injected into the guest's AWS provisioning by Phase 3 when the ENA build is enabled (the default on **OL6–OL10** — ENA Express generation; OL8/9/10 receive the host-resolved amzn-drivers latest via `ENA_DRIVER_VERSION`, and real AMI boot with an OL8/9/10 self-built driver is not yet E2E-verified). |

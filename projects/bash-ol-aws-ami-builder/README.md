...
The deepest legacy target is **Oracle Linux 5** (x86_64), added on the same synthesis model as OL6 plus a **full host-supply pipeline**: EL5 has no TLS 1.2 path, so every guest package beyond the ISO (the UEK R2 kernel set, the ENA build toolchain, the EPEL5 cloud-init 0.6.3 closure, gdisk, the ENA source tarball, and the AWS CLI v2 bundle) is downloaded on the build host and delivered through the standard `provision.d` files channel. See [section 9.8](#98-oracle-linux-5-support-experimental) for the mechanism and [section 10](#10-known-limitations-and-caveats) item 9 for the caveats. **Real EL5 anaconda contact and real EC2 boots are verified (2026-07-27)**: `t2` (Xen: `xvda` root, `vif` netfront) and `c5` (Nitro: NVMe root, self-built `ena 2.12.3g` driving the NIC), with growroot expanding the 7 GB image to the full EBS volume on both device-naming schemes — see TESTING.md "Boot-E2E evidence note (2026-07-27)".
...
| `env.properties.aws-ol6` | Build parameters for **Oracle Linux 6 Update 10** (x86_64) — **experimental / not shipped upstream**. Boots on Intel and AMD ≤ Zen2 (`c5a`) instance generations only — the frozen UEK4 kernel panics on AMD Zen3+ (measured 2026-07-18). See sections 9.7 and 10 for important caveats. |
| `env.properties.aws-ol5` | Build parameters for **Oracle Linux 5 Update 11** (x86_64) — **deepest legacy / not shipped upstream / real-boot E2E verified 2026-07-27** (`t2` Xen + `c5` Nitro; sosreport-backed). Full host-supply model (no in-guest network on EL5); the operator-measured SHA256 `ISO_CHECKSUM` is recorded in the template (2026-07-19; mirror-MD5 cross-checked — the media is terminal, so the value is permanent). See sections 9.8 and 10 for important caveats. |
| `setup-vmimport-role.sh` | One-time setup script that creates the `vmimport` IAM service role for AWS VM Import/Export. |
| `install-ena-driver.sh` | Self-contained script that builds and installs a pinned (OL6 → 2.9.1, OL7 → 2.17.2) or latest-resolved (OL8/9/10 — see SPEC B.9) Amazon ENA driver via DKMS, and states the ENA Express readiness of the installed module. Installs its own deps (EPEL, gcc/make, dkms, `kernel-uek-devel`) and can be run **standalone on a stock OL instance** for validation, or injected into the guest's AWS provisioning by Phase 3 when the ENA build is enabled (the default on **OL6–OL10** — ENA Express generation; OL8/9/10 receive the host-resolved amzn-drivers latest via `ENA_DRIVER_VERSION`, real AMI boot with the OL8/9/10 self-built driver was E2E-verified on 2026-07-13: `ethtool -i` = `2.17.2g` on real EC2 — see SPEC B.3.4 / TESTING.md). Additionally supports **OL5 (UEK R2 / `el5uek`)**: plain `make` only (no DKMS), host-pre-provisioned toolchain/headers/source (EL5 has no in-OS TLS 1.2 path), and the proven `el5uek` shim set applied and recorded per build — build-test proven and now wired into the OL5 AMI build path (Phase 3 host-stages the toolchain and source; see section 9.8) — see SPEC B.9 (OL5) and D.29. |
| `install-ssm-agent.sh` | Self-contained script that installs the Amazon SSM Agent RPM and enables it for boot (OL6-OL10 → `latest`; OL6 was pinned until 2026-07-13, re-pin policy in SPEC B.11). Fetches the RPM with `curl` and installs via `rpm -Uvh` (sidesteps the EL6 yum-over-HTTPS quirk); can be run **standalone** for the install+run test matrix (`tests/ssm/`), or injected into the guest's AWS provisioning by Phase 3 when the SSM install is enabled (the default). OL5 is a **measured exclusion** (unlike ENA/AWS CLI v2): the entire AWS-supported band is walled at both the package-install layer (xz/sha256 rpmlib vs EL5 rpm 4.4) and the kernel layer (triple-attested 3.2 floor vs terminal el5uek) — see SPEC D.31. |
| `install-awscli.sh` | Self-contained script that installs AWS CLI v2 (OL6 → pinned `2.17.51`, OL7/OL8 → `latest`) and excludes the OL-repo `awscli` (v1) via versionlock. Unzips the self-contained v2 bundle and installs it with the bundle's own `aws/install`; can be run **standalone** for the install+run test matrix (`tests/awscli/`), or injected into the guest's AWS provisioning by Phase 3 when the AWS CLI install is enabled (the default, OL6/OL7/OL8 — OL9/OL10 use their default package manager). Additionally supports **OL5** (ceiling pin 2.17.51 = the OL6 pin, measured same glibc-wall reason; bundle host-pre-staged — EL5 has no in-OS TLS 1.2 path) — install-test proven and now wired into the OL5 AMI build path (Phase 3 host-stages the bundle zip; see section 9.8) — see SPEC B.12 (OL5) and D.30. |
...
This role is required by AWS VM Import/Export to read the staged VMDK from S3. Run this **only once per AWS account**. The bucket name above matches the `S3_BUCKET` value shared by every `env.properties.aws-ol{N}` template in this directory, so the single role covers builds for OL5 / OL6 / OL7 / OL8 / OL9 / OL10 alike. The `S3_KEY_PREFIX` in each env file (e.g. `ol10-ami-import`) keeps the per-version VMDKs isolated within the bucket.
...
| `DISK_SIZE_GB` | Defaults to `7` (uniform across OL6-10, aligned with Oracle's own AWS AMIs; OL5 uses `10` — the host-staged artifact set is larger and EL5 has no growpart, so growth is handled by the baked one-shot `ol-aws-growroot` + cloud-init `resizefs` instead). The root partition auto-grows and `cloud-utils-growpart` is baked in, so instances launched with a larger EBS volume expand automatically — this is a floor, not a ceiling. See SPEC B.3.4. |
| `LINUX_FIRMWARE` | `no` in the OL8 template only. Removes the bulk `linux-firmware` **before the in-guest update** so the `UPDATE_TO_LATEST` transaction fits the 7 GB root (EL8's firmware is uniquely uncompressed, ~1.9 GB installed, and otherwise overflows it). Build-time headroom knob: the booted 2026-07-13 images were observed to carry `linux-firmware` again via the later provisioning flow (functionally benign); `linux-firmware-core` stays either way. See SPEC B.3.4. |
...
| `--skip-ena-driver` | Do **not** build/install the Amazon ENA driver in the guest. The default builds it on OL5-OL10 (AWS-optimized, ENA-Express-capable AMI; Nitro v4+/ENAv3); this switch produces a pure, unmodified OL AMI. |
| `--skip-ssm-agent` | Do **not** install the Amazon SSM Agent in the guest. The default installs and boot-enables it (OL6-OL10), so the AMI is AWS SSM Run Command compliant out of the box; this switch leaves the agent out. OL5 is a **measured exclusion** (SPEC D.31): the agent is forced off there regardless of this switch and the AMI name never carries an ssm marker. |
| `--skip-awscli` | Do **not** install AWS CLI v2 in the guest. The default installs it on OL5/OL6/OL7/OL8 (the standard CLI, since AWS CLI v1 is increasingly unsupported) and excludes the v1 package via versionlock; this switch leaves it out. OL9/OL10 are out of scope (they install AWS CLI v2 from their default package manager) and unaffected by this switch. |
...
| `--imds-support <mode>` | IMDS support baked into the AMI: `default` (IMDSv1+v2, `HttpTokens=optional`) or `v2.0` (IMDSv2-required, **OL7+ only**). Default `default`. OL6 + `v2.0` is rejected (its cloud-init 0.7.5 cannot use IMDSv2), and OL5 + `v2.0` likewise (its cloud-init 0.6.3 speaks plain IMDSv1 only). |
...
### 9.8 Oracle Linux 5 support (experimental)

OL5 is the deepest legacy target, built on the OL6 synthesis model plus a **full host-supply pipeline** (EL5's openssl 0.9.8e tops out at TLS 1.0, so the guest can reach no current repository at all).

- **Runtime-generated `distr/ol5-slim/`.** Four files (`env.properties`, `image-scripts.sh`, `ol5-ks.cfg`, `provision.sh`) are written from embedded heredocs, mirroring the OL6 flow with EL5 adjustments: anaconda-11.1 kickstart syntax (**no `%end`**, `key --skip`, `cdrom`, no `services`/`--ondisk`/`rootpw --lock`), an **ext3** root with LABEL-based partitions, GRUB Legacy serial console, and guest-side code kept **bash-3.2/POSIX clean** (EL5's bash predates `${var,,}`, `mapfile`, etc. — mechanically enforced by the `t025` test tier).
- **Full host-supply (`[OLAWS-OL5S1]`).** The build host downloads and verifies every guest artifact into `distr/ol5-slim/files/` — the frozen 11-RPM ENA build toolchain (byte-identical to the ENA test matrix's proven list), `kernel-uek`/`-devel`/`-firmware` (UEK R2, live-resolved from the frozen channel with a pinned fallback), the 9-RPM EPEL5 cloud-init 0.6.3 closure + `gdisk` (frozen immutable-archive NVRs), the ENA source tarball, and the AWS CLI v2 bundle zip. Upstream's standard files channel carries them into the guest (`provision.d` → `virt-customize --copy-in`), and a source-time executor installs them in load-bearing order: `modprobe.conf` aliases (so the kernel RPM's own `mkinitrd` bakes `nvme` in) → `DEFAULTKERNEL=kernel-uek` → one `rpm -Uvh` transaction → `/usr/src` staging → hard asserts (el5uek kernel present; initrd contains `nvme.ko` with one `mkinitrd` remediation retry; the grub default boots el5uek; cloud-init installed).
- **EL5-safe provisioning overrides (`[OLAWS-OL5S2]`).** The upstream `cloud::install_aws_packages` / `cloud::cloud_init` bodies are yum/dracut-based; appended redefinitions (bash last-definition-wins) replace them with verify-only EL5-safe versions. The dracut-based Nitro-initramfs hook is not injected on OL5, and the ENA hook's dracut drop-in lines are omitted.
- **`ec2-user` + cloud-init 0.6.3.** EPEL5's cloud-init cannot create users (`setup_user_keys` is getpwnam-only), so the kickstart `%post` pre-creates `ec2-user` (locked password, passwordless sudo) and `CLOUD_USER` is pinned. Its `DataSourceEc2` speaks plain IMDSv1 only, so `--imds-support v2.0` is rejected.
- **Root growth without growpart.** EL5 has no `cloud-utils-growpart`; a baked one-shot SysV script (`ol-aws-growroot`, S08) implements the **growpart decision model**: the PRIMARY execution criterion is the actual disk/partition state, re-evaluated every boot (sysfs geometry + on-disk table; `NOCHANGE` below a 1 MiB fudge; only a plain `Id=83` last partition is grown), and the attempt marker is SECONDARY — a reboot-loop breaker that self-heals on success, so a later EBS enlargement grows again. The write mechanism mirrors real growpart: `sfdisk -d` dump → single size-field edit (addressed by start sector — util-linux 2.13 misnames NVMe partitions) → `--no-reread --force` apply with backup/restore and a post-write verify. One reboot is required on this kernel line (`BLKPG_RESIZE_PARTITION` is 3.6+; UEK R2 is 3.0.36); cloud-init's `resizefs` then grows ext3. `gdisk` is staged as a diagnostic tool but never used against the MBR disk (GPT there would break GRUB Legacy).
- **virt-install disk bus.** Upstream pins the build VM disk to virtio-scsi, which the EL5 installer kernel cannot see; Phase 3 patches the bus to plain `virtio` (present in both the 5.11 installer kernel and UEK R2) for OL5 only.
- **ISO + checksum contract.** The 5.11 media lives on the kernel.org mirror under the original "Enterprise Linux" naming (`Enterprise-R5-U11-Server-x86_64-dvd.iso`). Upstream accepts SHA1/SHA256 checksums only while the mirror publishes MD5SUMS, so the operator computed the SHA256 once (cross-checking the mirror MD5 recorded in the env template) and it is now baked into `ISO_CHECKSUM` with its full provenance chain (MD5 + size + SHA256, measured 2026-07-19); the wrapper's fail-fast on an empty value stays as the safety net.
- **SSM Agent: measured exclusion.** Unlike ENA/AWS CLI v2, the agent's entire AWS-supported band is walled on EL5 (xz/sha256 rpmlib vs rpm 4.4, kernel 3.2 floor vs terminal el5uek — SPEC D.31); it is forced off regardless of flags.
- **E2E status.** **Verified end to end on 2026-07-27** (sosreport-backed; TESTING.md "Boot-E2E evidence note (2026-07-27)"): real EL5 anaconda completed the synthesized kickstart, the host-supplied rpm transaction and grub ownership held (`uname -r` = the staged `2.6.39-400.297.3.el5uek` on both instances), **Nitro boot is proven on `c5`** (NVMe root via the in-box `nvme`; `ethtool -i` = the self-built `ena 2.12.3g`) and the Xen path on `t2` (`xvda`/`vif`), with growroot expanding to the full EBS size on both naming schemes. The UEK R2 instance-generation boundary beyond `c5` is unmeasured — Phase 6 advisory tiers only.

See [section 10](#10-known-limitations-and-caveats) item 9 for the full list of OL5 caveats.
...
8. **Oracle Linux 6 specific limitations.**
   ...
   - **Phase A/B/C all verified (Phase C on 2026-07-13).** Static checks (osinfo-db entries, ISO checksum, repo HTTPS, dracut flags, cloud-init availability, upstream OL-version branches) and the OL6 ISO boot test (virt-install + isolinux + Anaconda 13.21.263 TUI) were verified first; the full end-to-end path — kickstart completion, provision.sh execution on OL6, cloud-init `ec2-user` creation (SSH login), and AMI launch on AWS **Nitro** (r5dn.large, NVMe root, self-built ENA 2.9.1g driving the NIC) — was verified on 2026-07-13 (sosreport-backed; see TESTING.md "Boot-E2E evidence note (2026-07-13)").
   - **Modern AMD instance generations panic at boot (measured 2026-07-18).** The frozen UEK4 kernel (`4.1.12`) hits `kernel BUG at arch/x86/kernel/alternative.c:708` (jump-label `text_poke` verification) during early boot on `c6a`/`c7a`/`c8a` (AMD Zen3/Zen4/Zen5) — before any module loads, so this is a kernel×CPU incompatibility, not an artifact of this pipeline. Measured working: Intel generations (through `r8i-flex`, 2026-07-13) and AMD ≤ Zen2 (`c5a.large`, 2026-07-18). The kernel is terminal/frozen; no fix is possible — launch OL6 AMIs on Intel or AMD Zen2-generation instance types only. See the TESTING.md "Boot-E2E evidence note (2026-07-18)".
...
9. **Oracle Linux 5 specific limitations.**
   - **EOL long past.** Premier Support ended in 2017; Extended Support ended in 2021. No updates of any kind have shipped for years; the repositories are terminal/frozen.
   - **Real-boot E2E verified (2026-07-27).** Measured instance types: **`t2` (Xen) and `c5` (Nitro)** — Nitro boot proven with NVMe root + the self-built ENA driving the NIC; growroot proven on real EBS on both device-naming schemes. The UEK R2 generation boundary beyond `c5` is unmeasured (the frozen 2.6.39 kernel may not boot newer generations — advisory only; cf. the measured OL6/UEK4 AMD Zen3+ boundary).
   - **No in-guest network, ever.** EL5 has no TLS 1.2 path; every package beyond the ISO is host-staged. The image cannot `yum update` after launch either — what is baked is what runs, permanently.
   - **cloud-init 0.6.3 limits.** Plain IMDSv1 only (IMDSv2-only AMIs rejected); no user creation (ec2-user is pre-created at install time); no growpart (replaced by the baked `ol-aws-growroot` one-shot on the growpart decision model — disk state PRIMARY, marker secondary — + `resizefs`); first boot includes **one automatic reboot** when the EBS volume is larger than the image, and any LATER volume enlargement grows again on the next boot.
   - **x86_64 / BIOS / MBR / GRUB Legacy / ext3 / UEK R2 only.** Every one of these is a hard EL5 constraint enforced by `distr::validate` or the kickstart shape.
   - **SSM Agent excluded (measured).** No wiring exists on OL5 and none can: the exclusion is triple-walled (SPEC D.31).
   - **Nitro compatibility rests on two pillars.** The in-box `nvme` driver in UEK R2 (measured from the kernel payload: v0.9-era with class-match PCI binding) and the self-built ENA (pinned at the swept build boundary). Whether this 3.0.36-base kernel boots current Nitro hardware is exactly what the pending E2E must prove; Xen-generation instances are the measured fallback.
   - **Production prohibited.** The strongest prohibition in this repository: verification, learning, and archaeology only.
...
This wrapper was **iteratively developed with Anthropic Claude (Sonnet 4.5)** in 2026-05, then refined based on real build runs against Oracle Linux 8 / 9 / 10 on AWS until all three versions completed end-to-end successfully. Oracle Linux 7 experimental support was added in a subsequent 2026-05 revision using Anthropic Claude (Opus 4.7); the OL7 patch mechanism has been verified (sed substitution, syntax integrity, idempotency) but **end-to-end OL7 AMI builds have not been validated by the author**. Oracle Linux 6 experimental support was added in a further 2026-05 revision using Anthropic Claude (Opus 4.7) after a two-phase verification session (Phase A: 9 static checks against osinfo-db, ISO/checksum, repo HTTPS, dracut, cloud-init; Phase B: virt-install + Anaconda 13.21.263 TUI boot test on libvirt 11.5 / qemu 10.0). The two OL6 runtime patches and the runtime-generated `distr/ol6-slim/` have been syntactically validated, but **end-to-end OL6 AMI builds (kickstart completion through AWS Nitro launch) had not been validated by the author at that time**. *(Superseded 2026-07-13: the current-generation pipeline completed real 7 GB builds AND real EC2 boots on ALL five majors — OL6-OL10, Nitro instance types up to the r8i generation, sosreport-backed. The OL7 and OL6 end-to-end gaps above are closed; see TESTING.md "Boot-E2E evidence note (2026-07-13)" and the CHANGELOG.)*

...
| `LINUX_FIRMWARE` | `no` in the OL8 template only. Drops the bulk `linux-firmware` package (unused on EC2 VMs); EL8's uniquely uncompressed ~1.9 GB firmware otherwise overflows the 7 GB root during `UPDATE_TO_LATEST`. OL9/OL10 already ship without firmware (upstream `KERNEL_MODULES=no` default); `linux-firmware-core` stays. See SPEC B.3.4. |

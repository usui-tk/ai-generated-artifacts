...
| `install-ena-driver.sh` | Self-contained script that builds and installs a pinned (OL6 → 2.9.1, OL7 → 2.17.0) or latest-resolved (OL8/9/10, evaluation/standalone only — see SPEC B.9) Amazon ENA driver via DKMS. Installs its own deps (EPEL, gcc/make, dkms, `kernel-uek-devel`) and can be run **standalone on a stock OL instance** for validation, or injected into the guest's AWS provisioning by Phase 3 when the ENA build is enabled (the default, OL6/OL7 only — OL8/9/10 stay on their in-distro driver in the AMI pipeline). |

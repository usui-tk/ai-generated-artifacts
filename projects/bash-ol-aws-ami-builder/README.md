...
- **Root growth without growpart.** EL5 has no `cloud-utils-growpart`; a baked one-shot SysV script (`ol-aws-growroot`, S08) repartitions the root partition to the disk end with the classic fdisk delete/recreate-at-same-start pattern (the MBR adaptation of the RHEL6-HVM gdisk workaround — `gdisk` is staged as a diagnostic tool but never used against the MBR disk, where writing GPT would break GRUB Legacy), marks itself done, and reboots once; cloud-init's `resizefs` then grows the ext3 filesystem.
...
9. **Oracle Linux 5 specific limitations.**
   ...
   - **cloud-init 0.6.3 limits.** Plain IMDSv1 only (IMDSv2-only AMIs rejected); no user creation (ec2-user is pre-created at install time); no growpart (replaced by the baked `ol-aws-growroot` one-shot + `resizefs`); first boot includes **one automatic reboot** when the EBS volume is larger than the image.

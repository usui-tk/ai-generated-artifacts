...
# -ol9.sh / -ol10.sh
#
# Build a *clean-core* Oracle Linux 10 container rootfs. Dedicated, self-contained
# OL10 script (no shared library, no config externalization -- by design).
#
...
#   [B] BUILDER      - a THROWAWAY Oracle "10-slim" container rootfs, driven via
#                      unshare+chroot. 10-slim ships ONLY microdnf, so the builder
#                      first `microdnf install dnf` (full dnf) using the
#                      builder-dedicated cleancore.repo below (its own bundled
#                      repo configs are removed first, never read). Its EL10 rpm
...
#   - Builder image (OL10 "10-slim" rootfs tarball), Oracle official:
#       https://github.com/oracle/container-images/tree/0218ab4ba2f820b1b978dcc5a76435040397a472/10-slim
#       https://github.com/oracle/container-images/raw/0218ab4ba2f820b1b978dcc5a76435040397a472/10-slim/oraclelinux-10-slim-amd64-rootfs.tar.xz
#   - Package set (kickstart %packages), Oracle official:
#       https://github.com/oracle/oracle-linux/tree/main/oracle-linux-image-tools/distr/ol10-slim
#       https://github.com/oracle/oracle-linux/blob/main/oracle-linux-image-tools/distr/ol10-slim/ol10-ks.cfg
...
# ----------------------------------------------------------------------------
# Usage:   bash build-cleancore-ol10.sh [output.tar.gz]
...
# "10-slim" image by its FLOATING tag (always the latest 10.x slim), pulled over the
# OCI registry v2 API (curl-only, or a container runtime if present); (2) FALLBACK
# the SAME 10-slim rootfs pinned in oracle/container-images at CI_COMMIT (byte-stable
...
OL_TAG="${OL_TAG:-10-slim}"                  # floating: latest 10.x slim
CI_COMMIT="0218ab4ba2f820b1b978dcc5a76435040397a472"   # oracle/container-images pin (fallback)
BUILDER_URL="https://github.com/oracle/container-images/raw/${CI_COMMIT}/10-slim/oraclelinux-10-slim-amd64-rootfs.tar.xz"

WORK="${WORK:-/tmp/cleancore-ol10}"
OUT_TARBALL="${1:-${WORK}/cleancore-ol10-rootfs.tar.gz}"
...
INCLUDE=( dnf oraclelinux-release-el10 oracle-epel-release-el10
  # python3-dnf-plugin-versionlock (default; in OL10 baseos): package-pinning plugin
...
# floating tag (e.g. 10-slim) is always the latest N.x slim. Returns 0 on success,
...
# ╔════════════════════════════════════════════════════════════════════════╗
# ║ [A] HOST — acquire the builder rootfs. PRIMARY: the floating 10-slim tag  ║
# ║     from the Oracle container registry (latest 10.x; OCI v2 / curl).      ║
# ║     FALLBACK: the pinned 10-slim git-raw rootfs (byte-stable) if unreachable.║
# ╚════════════════════════════════════════════════════════════════════════╝
log "[A] acquiring the EL10 builder rootfs"
rm -rf "${WORK}"
mkdir -p "${BUILDER}"
if oci_pull_rootfs "${OL_REGISTRY}" "${OL_IMAGE}" "${OL_TAG}" "${BUILDER}"; then
  log "[A] using the floating ${OL_TAG} image from ${OL_REGISTRY#https://}/${OL_IMAGE} (latest 10.x)"
elif curl -fsSL --retry 2 -o "${WORK}/builder.tar.xz" "${BUILDER_URL}" 2>/dev/null; then
  log "[A] registry pull unavailable; falling back to the pinned 10-slim rootfs (${BUILDER_URL})"
  tar -C "${BUILDER}" -xJf "${WORK}/builder.tar.xz"
  rm -f "${WORK}/builder.tar.xz"
else
  log "[A] ERROR: could not acquire the 10-slim builder rootfs (registry + pinned both failed)"; exit 1
fi
...
# ║     removed, never read). 10-slim has no find/xargs, so all file edits      ║
# ║     happen here on [A].                                                    ║
# ╚════════════════════════════════════════════════════════════════════════╝
...
# ║     (4A.1) microdnf install dnf  -- 10-slim ships only microdnf; install   ║
...
cc_search_ready() {
  ...
  if [ -n "${rt}" ]; then
    local tag="cleancore-ol10-selftest:tmp" rc

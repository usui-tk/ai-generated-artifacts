...
# -ol9.sh / -ol10.sh
#
# Build a *clean-core* Oracle Linux 9 container rootfs. Dedicated, self-contained
# OL9 script (no shared library, no config externalization -- by design).
#
...
#   [B] BUILDER      - a THROWAWAY Oracle "9-slim" container rootfs, driven via
#                      unshare+chroot. 9-slim ships ONLY microdnf, so the builder
#                      first `microdnf install dnf` (full dnf) using the
#                      builder-dedicated cleancore.repo below (its own bundled
#                      repo configs are removed first, never read). Its EL9 rpm
...
#   - Builder image (OL9 "9-slim" rootfs tarball), Oracle official:
#       https://github.com/oracle/container-images/tree/0218ab4ba2f820b1b978dcc5a76435040397a472/9-slim
#       https://github.com/oracle/container-images/raw/0218ab4ba2f820b1b978dcc5a76435040397a472/9-slim/oraclelinux-9-slim-amd64-rootfs.tar.xz
#   - Package set (kickstart %packages), Oracle official:
#       https://github.com/oracle/oracle-linux/tree/main/oracle-linux-image-tools/distr/ol9-slim
#       https://github.com/oracle/oracle-linux/blob/main/oracle-linux-image-tools/distr/ol9-slim/ol9-ks.cfg
...
# ----------------------------------------------------------------------------
# Usage:   bash build-cleancore-ol9.sh [output.tar.gz]
...
BUILDER_URL="https://github.com/oracle/container-images/raw/${CI_COMMIT}/9-slim/oraclelinux-9-slim-amd64-rootfs.tar.xz"

WORK="${WORK:-/tmp/cleancore-ol9}"
OUT_TARBALL="${1:-${WORK}/cleancore-ol9-rootfs.tar.gz}"
...
# Package manifest (upstream ol9-slim ol9-ks.cfg %packages; @core implicit base).
# @core (Mandatory) already pulls oraclelinux-release-el9, so it is not listed.
...
INCLUDE=( @core
  ...
  elfutils-debuginfod-client freetype glibc-langpack-en graphite2
  grub2-tools-efi grub2-tools-extra harfbuzz libpng openssl-pkcs11
  python-unversioned-command rpm-plugin-systemd-inhibit rsyslog-logrotate zstd )

# ol9-ks.cfg "-pkg" removals. With group_package_types=mandatory (below) the
# @core *Default* members (NetworkManager-team/-tui, the iwl* firmware, lshw,
# lsscsi, microcode_ctl, prefixdevname, sg3_utils*) are not pulled in the first
# place; the *Mandatory* ones the ks drops (dnf-plugin-spacewalk, iproute-tc,
# irqbalance, parted, rhn-*, rng-tools, sssd-*) are excluded here, plus the ks's
# explicit langpack/compat trims. Firmware (option A): excluded via the
# *firmware* glob. kernel-uek is a *Mandatory* @core member on EL9 but a
# clean-core container ships no kernel (matching OL7), so it is excluded too.
EXCLUDE="dnf-plugin-spacewalk,iproute-tc,irqbalance,parted,\
rhn-client-tools,rhn-setup,rhnlib,rhnsd,rng-tools,sssd-common,sssd-kcm,\
NetworkManager-team,NetworkManager-tui,lshw,lsscsi,microcode_ctl,prefixdevname,\
sg3_utils,sg3_utils-libs,gawk-all-langpacks,glibc-all-langpacks,\
glibc-gconv-extra,libcap-ng-python3,libxcrypt-compat,*firmware*,kernel-uek"

BUILDER="${WORK}/builder"      # [B] throwaway EL9-native builder rootfs
OUT="/cleancore"               # [C] installroot path INSIDE the builder
DELIV="${BUILDER}${OUT}"       # [C] deliverable rootfs as seen from [A]

log() { printf '%s [cleancore-ol9] %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$*"; }


# ╔════════════════════════════════════════════════════════════════════════╗
# ║ [A] HOST — acquire the builder rootfs (download + extract; no runtime)    ║
# ╚════════════════════════════════════════════════════════════════════════╝
log "[A] downloading 9-slim builder rootfs"
rm -rf "${WORK}"
mkdir -p "${BUILDER}"
...
# ║     removed, never read). 9-slim has no find/xargs, so all file edits      ║
# ║     happen here on [A].                                                    ║
# ╚════════════════════════════════════════════════════════════════════════╝
...
# ║     (4A.1) microdnf install dnf  -- 9-slim ships only microdnf; install   ║
...
cc_search_ready() {
  ...
  if [ -n "${rt}" ]; then
    local tag="cleancore-ol9-selftest:tmp" rc
...
# ───────────────────────────────────────────────────────────────────────────
log "[A] clean-core OL9: ${PKGS} packages, ${SIZE} (unpacked)"

...
# "9-slim" image by its FLOATING tag (always the latest 9.x slim), pulled over the
# OCI registry v2 API (curl-only, or a container runtime if present); (2) FALLBACK
# the SAME 9-slim rootfs pinned in oracle/container-images at CI_COMMIT (byte-stable
...
OL_TAG="${OL_TAG:-9-slim}"                   # floating: latest 9.x slim
...
# Package manifest (slim-aligned, NOT @core). The pristine ol9-slim image is the
...
# essentials a test base needs. systemd is unavoidable on EL9 once full dnf is present
...
INCLUDE=( dnf oraclelinux-release-el9 oracle-epel-release-el9
  # python3-dnf-plugin-versionlock (default; in OL9 baseos): package-pinning plugin
...
# floating tag (e.g. 9-slim) is always the latest N.x slim. Returns 0 on success,
...
# ╔════════════════════════════════════════════════════════════════════════╗
# ║ [A] HOST — acquire the builder rootfs. PRIMARY: the floating 9-slim tag   ║
# ║     from the Oracle container registry (latest 9.x; OCI v2 / curl).       ║
# ║     FALLBACK: the pinned 9-slim git-raw rootfs (byte-stable) if unreachable.║
# ╚════════════════════════════════════════════════════════════════════════╝
log "[A] acquiring the EL9 builder rootfs"
rm -rf "${WORK}"
mkdir -p "${BUILDER}"
if oci_pull_rootfs "${OL_REGISTRY}" "${OL_IMAGE}" "${OL_TAG}" "${BUILDER}"; then
  log "[A] using the floating ${OL_TAG} image from ${OL_REGISTRY#https://}/${OL_IMAGE} (latest 9.x)"
elif curl -fsSL --retry 2 -o "${WORK}/builder.tar.xz" "${BUILDER_URL}" 2>/dev/null; then
  log "[A] registry pull unavailable; falling back to the pinned 9-slim rootfs (${BUILDER_URL})"
  tar -C "${BUILDER}" -xJf "${WORK}/builder.tar.xz"
  rm -f "${WORK}/builder.tar.xz"
else
  log "[A] ERROR: could not acquire the 9-slim builder rootfs (registry + pinned both failed)"; exit 1
fi

...
assert_match "${s1}" 'grub default boots \$\{defver:-<none>\}, not the staged' "s1: EXACT-kver grub default assert (record #8: substring matching let the media kernel keep the default)"
assert_match "${s1}" 'removing non-target kernel' "s1: non-target kernels (RHCK + media UEK) are removed (OL-parity)"
assert_match "${s1}" 'cloning the current default entry' "s1: grub entry for the staged kver is guaranteed (clone-from-default when %post grubby failed)"
...
assert_match "${main}" 'ol-aws-ami-builder OL5 env-defaults PATCH' "wiring: env.properties.defaults declare-gA guarded for the EL5 guest (2026-07-20 first-contact finding)"
...
# grub ownership (record #8): the REAL OL5S1 grub section executed on
# fixtures -- clone-from-default, prune, default re-point, exact assert.
printf '%s\n' "${s1}" > "${WORK}/s1.body"
sed -n '/^# grub.conf ownership (GRUB Legacy):/,/grub default boots the staged kernel/p' "${WORK}/s1.body" > "${WORK}/grubown.sh"
if [ -s "${WORK}/grubown.sh" ] && bash -n "${WORK}/grubown.sh" 2>/dev/null; then
  t_pass "grub(behavioral): ownership section extracted from the real OL5S1 body ($(wc -l < "${WORK}/grubown.sh") lines)"
else
  t_fail "grub(behavioral): extraction failed"
fi
gkv="2.6.39-400.297.3.el5uek"
grub_run() {
  ( set -eu
    # shellcheck disable=SC2034
    kv="${gkv}"
    ...
    OL5_BOOT_DIR="$1"
    # shellcheck disable=SC2034
    OL5_GRUB_CONF="$1/grub/grub.conf"
    # shellcheck disable=SC1090,SC1091
    . "${WORK}/grubown.sh"
  )
}
mkdir -p "${WORK}/gb1/grub"
touch "${WORK}/gb1/vmlinuz-${gkv}" "${WORK}/gb1/initrd-${gkv}.img"
cat > "${WORK}/gb1/grub/grub.conf" <<'EOF_G1'
...
title Oracle Linux Server (2.6.39-400.215.10.el5uek)
	root (hd0,0)
	kernel /vmlinuz-2.6.39-400.215.10.el5uek ro root=LABEL=/ console=tty0 console=ttyS0,115200n8
	initrd /initrd-2.6.39-400.215.10.el5uek.img
title Oracle Linux Server-base (2.6.18-419.el5)
	root (hd0,0)
	kernel /vmlinuz-2.6.18-419.el5 ro root=LABEL=/ console=ttyS0,115200n8
	initrd /initrd-2.6.18-419.el5.img
EOF_G1
grub_run "${WORK}/gb1" >/dev/null 2>&1
rc=$?
n_title="$(grep -c '^title' "${WORK}/gb1/grub/grub.conf")"
if [ "${rc}" = "0" ] && [ "${n_title}" = "1" ] \
   && grep -q "^default=0$" "${WORK}/gb1/grub/grub.conf" \
   && grep -q "vmlinuz-${gkv} ro root=LABEL=/ console=tty0 console=ttyS0,115200n8" "${WORK}/gb1/grub/grub.conf"; then
  t_pass "grub(behavioral): media-default fixture -> cloned kv entry (args preserved), stale entries pruned, default=0"
else
  t_fail "grub(behavioral): T1 rc=${rc} titles=${n_title}"
fi
grub_run "${WORK}/gb1" >/dev/null 2>&1
rc=$?
n_title="$(grep -c '^title' "${WORK}/gb1/grub/grub.conf")"
if [ "${rc}" = "0" ] && [ "${n_title}" = "1" ]; then
  t_pass "grub(behavioral): re-run is idempotent (single entry, rc=0)"
else
  t_fail "grub(behavioral): T2 rc=${rc} titles=${n_title}"
fi
mkdir -p "${WORK}/gb3/grub"
touch "${WORK}/gb3/vmlinuz-${gkv}" "${WORK}/gb3/initrd-${gkv}.img"
touch "${WORK}/gb3/vmlinuz-2.6.39-400.215.10.el5uek" "${WORK}/gb3/initrd-2.6.39-400.215.10.el5uek.img"
{
  echo "default=0"
  echo "timeout=5"
  printf 'title A (2.6.39-400.215.10.el5uek)\n\troot (hd0,0)\n\tkernel /vmlinuz-2.6.39-400.215.10.el5uek ro root=LABEL=/\n\tinitrd /initrd-2.6.39-400.215.10.el5uek.img\n'
  printf 'title B (%s)\n\troot (hd0,0)\n\tkernel /vmlinuz-%s ro root=LABEL=/\n\tinitrd /initrd-%s.img\n' "${gkv}" "${gkv}" "${gkv}"
} > "${WORK}/gb3/grub/grub.conf"
grub_run "${WORK}/gb3" >/dev/null 2>&1
rc=$?
if [ "${rc}" = "0" ] && grep -q "^default=1$" "${WORK}/gb3/grub/grub.conf" \
   && [ "$(grep -c '^title' "${WORK}/gb3/grub/grub.conf")" = "2" ]; then
  t_pass "grub(behavioral): kv entry already present -> default re-pointed to it; surviving entries kept"
else
  t_fail "grub(behavioral): T3 rc=${rc}"
fi
mkdir -p "${WORK}/gb4/grub"
cp "${WORK}/gb3/grub/grub.conf" "${WORK}/gb4/grub/grub.conf"
touch "${WORK}/gb4/vmlinuz-2.6.39-400.215.10.el5uek"
grub_run "${WORK}/gb4" >/dev/null 2>&1
rc=$?
if [ "${rc}" != "0" ]; then
  t_pass "grub(behavioral): staged vmlinuz absent -> loud FATAL (no silent wrong-kernel default)"
else
  t_fail "grub(behavioral): T4 unexpectedly passed"
fi
...
assert_match "${envf}" '^SELINUX="enforcing"$' "env: SELinux enforcing (adjudicated 2026-07-20 plan B: measure real denials; permissive is the measured fallback)"
assert_match "${envf}" '^DISK_SIZE_GB="7"$' "env: 7 GB floor (adjudicated 2026-07-20, OL6-10 aligned; measured usage ~1.7 GB; growroot makes it a floor, not a ceiling)"

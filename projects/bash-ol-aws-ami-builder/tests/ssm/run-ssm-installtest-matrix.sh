...
#   * kernel axis -- NOT faithful in a container: `uname -r` is the RUNNER's
#     kernel (shared by the container), not the OL's UEK, so the Go runtime's
#     minimum-kernel never trips on a modern runner. The matrix RECORDS `kver` as
#     per-entry context and, instead of pretending to test the kernel, surfaces a
#     STATIC kernel-axis proxy from each release's go.mod `go` directive (see
#     go_min_kernel + the release list's go_version). The faithful kernel verdict
#     needs a kernel-matched runner or a real instance (the B-T8 analog).
#
...
#
# Ledger dedup key (osmajor, ssm_version, kver), kver PRIMARY (a new kernel
# re-tests all versions); glibc + go_version recorded per entry.
#
# Usage:
...
# runs on the RUNNER's kernel (recorded as kver), so a UEK probe would be
# misleading. The SSM gate is therefore the SSM-VERSION probe: run the OL if the
# latest upstream SSM version is newer than the highest the ledger has tested for
# it (or the OL has no ledger entry). A new runner kernel is handled by the
# kver-PRIMARY ledger dedup (a new kver re-tests every version).
probe_latest_ssm() {
...
" "${LEDGER}" "${ol}" "${ver}" "${live_kver}"; then
    ...
    if [ -z "${rjson}" ]; then
      rjson="{\"status\":\"fail\",\"osmajor\":\"${ol}\",\"ssm_version\":\"${ver}\",\"kver\":\"${live_kver}\",\"glibc\":\"\",\"installed_version\":\"\",\"ran\":false,\"run_method\":\"\",\"reason\":\"no result line (infrastructure error)\"}"
    fi
    st="$(printf '%s' "${rjson}" | python3 -c "import json,sys; print(json.load(sys.stdin).get('status',''))" 2>/dev/null || true)"
    if [ "${st}" != "ok" ]; then
...
for ol, entries in by_ol.items():
    ...
    lines.append("")
    lines.append(f"AWS requirement: SSM Agent **>= {min_ver}** (from 2026-06-16 Run Command drops the legacy "
                 "`ec2messages` endpoint; agents at/above this use `ssmmessages`).")
    lines.append("")
    lines.append("**Fidelity.** `status=ok` means the RPM installed AND the agent binary ran locally on this "
                 "`(kernel, glibc)`. The **glibc** axis is faithful (the container's real OL glibc gates a dynamic "
                 "version). The **kernel** axis is NOT tested in a container -- `kver` is the runner's kernel, so "
                 "the recorded run reflects it, not the OL's UEK; the per-version **go_version / min_kernel(proxy)** "
                 "columns are the static kernel-axis signal (the Go toolchain's minimum kernel from the release's "
                 "go.mod). A faithful kernel verdict needs a kernel-matched runner or a real instance.")
    lines.append("")
    for kv in kvers:
        ...
        if not max_ok:
            verdict = "**none** -- nothing installed+ran on this (kernel, glibc)"
            ...
            verdict = (f"**ec2messages-only** -- max install+run `{max_ok}` < `{min_ver}`: the required version will "
                       "not install/run here, so this (kernel, glibc) CANNOT be remediated by an agent update and is "
                       "affected by the 2026-06-16 Run Command ec2messages deprecation")
        lines.append(f"## Latest kernel {kv}  (glibc {glibc or '?'})")
        lines.append("")
        lines.append(f"Verdict: {verdict}.")
        lines.append("")
        lines.append("| SSM version | status | ran | glibc | go_version | min_kernel(proxy) | note |")
        lines.append("|---|---|---|---|---|---|---|")

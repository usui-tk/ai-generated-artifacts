...
`modinfo` (kmod), and `python3`): **749 passed, 0 skipped, 0 failed** across
...
ol10-epel = 37, ol5-ena = 40, ol5-awscli = 26, ol5-build = 115).
Optional-tool degradations are the only way to see a skip: without
`ksvalidator` B-T4 contributes a skip (-> 748/1); without `modinfo` the
ena-uek-detect inbox-report assertions fold into one skip (-> 746/1); without

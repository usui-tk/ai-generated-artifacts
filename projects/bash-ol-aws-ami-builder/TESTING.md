...
`modinfo` (kmod), and `python3`): **561 passed, 0 skipped, 0 failed** across
...
`ksvalidator` B-T4 contributes a skip (-> 560/1); without `modinfo` the
ena-uek-detect inbox-report assertions fold into one skip (-> 558/1); without

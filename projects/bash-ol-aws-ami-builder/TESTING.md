...
`modinfo` (kmod), and `python3`): **602 passed, 0 skipped, 0 failed** across
**23 tiers** (B-T1 parse = 52, B-T2 ShellCheck = 47, B-T3 unit = 59,
...
ol10-epel = 37, ol5-ena = 39).
Optional-tool degradations are the only way to see a skip: without
`ksvalidator` B-T4 contributes a skip (-> 601/1); without `modinfo` the
ena-uek-detect inbox-report assertions fold into one skip (-> 599/1); without

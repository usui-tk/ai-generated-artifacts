...
`modinfo` (kmod), and `python3`): **797 passed, 0 skipped, 0 failed** across
...
ol10-epel = 37, ol5-ena = 40, ol5-awscli = 30, ol5-build = 158).
Optional-tool degradations are the only way to see a skip: without
`ksvalidator` B-T4 contributes a skip (-> 796/1); without `modinfo` the
ena-uek-detect inbox-report assertions fold into one skip (-> 794/1); without

...
`modinfo` (kmod), and `python3`): **822 passed, 0 skipped, 0 failed** across
**25 tiers** (B-T1 parse = 54, B-T2 ShellCheck = 49, B-T3 unit = 74,
command-mock = 15, B-T4 kickstart = 1, env-parity = 55, idempotency = 15,
...
ol10-epel = 37, ol5-ena = 40, ol5-awscli = 31, ol5-build = 166).
Optional-tool degradations are the only way to see a skip: without
`ksvalidator` B-T4 contributes a skip (-> 806/1); without `modinfo` the
ena-uek-detect inbox-report assertions fold into one skip (-> 804/1); without
...
> **Boot-E2E evidence note (2026-07-27) — the first real OL5 boots, and the
> OL6 CHECK-6 live proof.** The 2026-07-27 generation (built on an **EC2
> nested-virtualization host**, Fedora 44 — the record-#9 environment)
> registered and booted both legacy majors, sosreport-backed:
> **OL5 `ami-0f9a04a9b4683fd44`** booted on **`t2` (Xen)** and **`c5`
> (Nitro)** — the first real EL5 anaconda contact and the first real EC2
> boots of the OL5 path. Per the sosreports: (1) both instances run the
> staged **`2.6.39-400.297.3.el5uek`** (the record-#8 grub-ownership
> invariant, live); (2) on `c5` the root is NVMe (`nvme0n1p2`, in-box nvme
> driving it) and `ethtool -i eth0` shows the self-built **`ena 2.12.3g`**
> driving the NIC — Nitro boot is PROVEN; on `t2` the Xen path works
> (`xvda2` root, `vif` netfront); (3) **growroot is proven on real EBS on
> BOTH device-naming schemes**: the 7 GB image grew to the full ~124 GiB
> volume (`nvme0n1p2` and `xvda2` both ~124 GiB used-2%) — the fdisk MBR
> pattern + one reboot + resizefs, end to end; (4) kernel logs are clean
> (no oops/panic/trace) on both. **SELinux shipped `permissive` despite
> the enforcing adjudication** — the run-9 sosreports exposed the OL5 ks
> `selinux --permissive` hardcode (fixed the same day: the directive is
> now templated from `${SELINUX}`; record #10). Silver lining: permissive
> logged the complete would-be-denial set, and it is TWO benign entries
> per instance (`ifconfig_t` and `restorecon_t` read/write on an
> `initrc_t` unix_dgram syslog socket) — the plan-B enforcing measurement
> now has a preview and stays scheduled for the next boot generation.
> **OL6 `ami-03be4f95afdc8cc6f`** booted on **`c5` and `c6i`** running
> **`4.1.12-124.48.6.el6uek`** — the very kernel CHECK 6 verified after
> the LIVE two-kernel scenario (the host's broken libvirt NAT DNS left
> the media UEK installed at anaconda; provisioning installed the target
> and removed the rest; CHECK 6 proved the default) — with `ena 2.9.1g`
> on both, **SELinux enforcing and zero AVC denials**, the SSM Agent
> running, and awscli 2.17.51 (the record-#9 ensure-unzip fix, live).
> Measured-instance ledger after this note: OL5 = `t2` (Xen) + `c5`
> (Nitro v2); OL6 adds `c5`/`c6i` (Intel Nitro v2/v4) to the existing
> Intel-through-`r8i-flex` + AMD-≤-Zen2 (`c5a`) set. The OL5 UEK R2
> instance-generation boundary beyond `c5` remains unmeasured (advisory
> tiers only).
...
| B-T6 idempotency | L2 | implemented | `tests/t007_idempotency.sh` (structural): each of the 14 `[ol-aws-ami-builder PATCH ...]` markers (incl. the `sos-package` kickstart patch and the opt-in `amazon-time-sync` hook) is fronted by a `grep -Fq` guard; runtime apply-twice is B-T7/B-T8 |
...
| B-T (ol5 ena) | L0/L1 | implemented | `tests/t023_ol5ena.sh`: the opt-in OL5/`el5uek` ENA integration (SPEC B.9 OL5 paragraph + D.29) and the ledger merge mode. Structural pins: the installer's OL5 branch (pin 2.9.1, osmajor dispatch, pre-provision + pre-stage contracts, plain-make-only, initramfs skip, the `shims` result-JSON field) and every proven `el5uek` shim pattern string (S1–P7 — drift in one is caught here, not by a live OL5 build), plus the EL5-guest safety rule (no `sort -V` in OL5 code paths; comments excluded); matrix pins: the default OL set stays 6–10, `pin_for(5)`/`uekr_for(5)`, `--ol 5` acceptance, the frozen 11-RPM toolchain closure + kernel-devel fallback pin, the gate `.x86_64` normalization, ledger schema 1.2 + `shims`, the RESULTS-ol5 scope paragraph, and the `--merge-from` usage. Behavioural (hermetic, python3 only): the three adjudicated merge cases — adopt + same-status-keeps-incumbent, different-status hard error with the base ledger untouched, and `--merge-prefer theirs` resolution; 39 asserts |
| B-T (ol5 awscli) | L0/L1 | implemented | `tests/t024_ol5awscli.sh`: the opt-in OL5 AWS CLI v2 integration (SPEC B.12 OL5 paragraph + D.30) and the awscli ledger merge mode. Structural pins: the installer OL5 branch (ceiling pin 2.17.51 = the OL6 pin, osmajor dispatch, the no-in-guest-yum bypass, the AWSCLI_OL5_KVER probed-kver contract, the bundle pre-stage contract), the ABSENCE of the W1 wrapper (re-adjudicated out after the D.30 /proc correction), and the shared-code EL5 safety (no `sed -E` outside comments; the kver `sort -V` stderr-silenced); matrix pins: the default OL set stays 6-8, `pin_for(5)`, `--ol 5` acceptance, the staging function + pinned kver fallback, the `--merge-from` usage, and the RESULTS-ol5 scope paragraph. Behavioural (hermetic, python3 only): the three adjudicated merge cases on this matrix dedup key; 25 asserts |
| B-T (ol5 build) | L1/L2 | implemented | `tests/t025_ol5build.sh`: the OL5 AMI build target's runtime-synthesized artifacts + wrapper wiring. Extracts the embedded `distr/ol5-slim` templates (env / image-scripts / kickstart / provision) and the `[OLAWS-OL5S1]` executor body and asserts: the EL5/anaconda-11.1 kickstart shape (zero `%end`, `key --skip`, `cdrom`, ext3 + LABEL parts, RHEL6+ tokens absent, ec2-user pre-creation, the fdisk-pattern growroot one-shot, serial console, sysprep stubs); the bash-3.2 safety of every guest-side block (mechanical scan for `${var,,}`/`${var^^}`/`mapfile`/`declare -A`/`|&` on non-comment lines); the OL5S1 load-bearing order (modprobe aliases < DEFAULTKERNEL < rpm transaction < /usr/src staging) + hard asserts; the provision EL5 discipline (no `common::distr_cleanup`, `without-password`, rp_filter 0, poisoned DRACUT_CMD); the host-supply manifests (builder toolchain list BYTE-IDENTICAL to the matrix's `OL5_TOOLCHAIN_RPMS`; the 10 frozen EPEL5 NVRs incl. cloud-init 0.6.3 + gdisk); the wrapper wiring (behavioral `Enterprise-R5-U11` parse, IMDSv2-only rejection, SSM forced-off, NVM01 skip, awscli guard, disk-bus patch); the REAL extracted `_p3_validate_ks` passing the REAL extracted kickstart and failing `%end`-injected / sos-dropped mutations; the `env.properties.aws-ol5` template invariants; and the `ol-aws-growroot` one-shot on the **growpart decision model** (adjudicated: geometry PRIMARY / attempt-marker SECONDARY) — structural pins (sfdisk dump-edit-apply with `--no-reread --force`, start-sector addressing for the util-linux-2.13 NVMe-naming quirk, Id=83 guard, backup/restore, post-write verify, the BLKPG-3.6+ reboot grounding) plus a **behavioral harness** (fake sysfs + mocked df/sfdisk/reboot driving the real extracted script through five paths: grow/apply/verify/reboot, post-grow NOCHANGE + marker self-heal, marker-as-secondary loud no-retry, sub-fudge NOCHANGE, Id-guard refusal); plus the kickstart-grammar pins from the D.32 verification record (firewall --ssh grounded against real pykickstart 0.43.9; the single modern-ksvalidator --nobase divergence normalized-with-rationale in B-T4) and the 2026-07-20 first-contact set (env-defaults declare-gA guard + the guest-bound env-concat channel scans, and the record-#2 patch×gate INTEGRATION behaviorals -- the real extracted `_ol5_patch_env_defaults`/`_ol5_scan_bash32_hostile` composed on fixtures: patched=0 findings, raw declare -gA caught, shape-strict exemption, idempotent re-apply); the record-#5 closure set (the real `_ol5_stage_closure_gate` on 25 real-shape stub RPMs: PASS, libicu-provider-removed FAIL, glibc exact-NVR-regression FAIL; matrix<->builder set-equivalence with the three adjudicated divergences); the record-#6 build-info behaviorals (the real `distr::write_build_info` on fixtures: artifacts + EPOCHNUM-free epoch csv + grub-derived kernel.txt, el5uek fallback, loud FATAL when underivable, the upstream mv+rmdir sequence); the record-#8 grub-ownership set (the real OL5S1 grub section on anaconda-shaped fixtures: media-default clone with args preserved + stale-entry prune + default re-point, idempotent re-run, existing-entry default re-point, loud FATAL when the staged vmlinuz is absent; exact-kver assert / non-target-kernel removal / staged-rpm kv derivation / CHECK-6 structural pins incl. its OL5/OL6 GRUB-Legacy coverage), the run-8 message-truthfulness pins (awscli default-ON span OL5-OL8; plain-make ENA provenance labels on OL5), the record-#9 EROFS-guard pins (upstream resolver-cleanup patch marker + the OL5 cleanup guard), the record-#10 pins (the OL5 ks selinux directive is templated from the adjudicated env -- the hardcoded --permissive silently overrode enforcing -- and the awscli SUMMARY gate includes OL5) and the adjudicated env pins (SELINUX=enforcing, DISK_SIZE_GB=7); 166 asserts |

...
| `research_reference_drift_test.py` (T56) | The research-reference drift guard (2026-08 alignment remediation). Scans the current normative files (SPEC, README/README.ja, TESTING, this file, and the main script including comments) and rejects the six retired statement families: Server 2016 Setup-DU non-existence, per-OS-generation Kind-absence and the retired Forbidden-Kind-axis phrasing (English and Japanese), stale research-report paths missing the `documents/` prefix, Digest-as-universal-key, universal boot.wim LCU-impossibility (bare impossibility phrasing counts only with boot.wim nearby), and PCA2023 boot-manager claims using the retired signing verb (English and Japanese). CHANGELOG is out of scope; Historical/Superseded regions (heading, Status marker, or paragraph label) are excluded. Machinery is proven against built-in synthetic samples first, so the repository scan cannot go vacuous. Also pins the STAGE 3 trigger surfaces to each other (workflow `on:` exactly `workflow_dispatch`; TESTING Current-trigger paragraph states dispatch-only). Guards reintroduction only -- it does not turn the research report into policy; 32 assertions | On every change to the normative documents, the main script's comments, or the STAGE 3 workflow triggers; on every CI run | No |
...
tests/
  ...
  research_reference_drift_test.py       T56 (research-reference drift guard + STAGE 3 trigger pin, 32)

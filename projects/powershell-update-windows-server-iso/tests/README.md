...
| `servicing_dependency_parser_test.py` (T12) | Offline self-verification of the wsusscn2 parser pipeline Stages 3 and 4. Drives `ConvertFrom-OfflineSyncPackage` + `New-ServicingDependencyDatabase` against the small committed fixture `fixtures/servicing-dependency/package.xml`, then structurally compares against `fixtures/servicing-dependency/expected-output.json` (after stripping environmental `scriptVersion` / `scriptTag` / `generatedAt` / `sourceCab` fields). 22 assertions covering: stats parity, scope-filter admit/reject (Product/Classification/recency), Category-Update detection, FileLocation -> payload-URL join with orphan-digest accounting, Microsoft-prose absence in both fixture and parser output (SPEC §B.19.5 hard rule), and the kbIds field being emitted for every update (M1 populate). 23 assertions. | After every change to Stage 3 or Stage 4 of the wsusscn2 parser pipeline, the scope-filter GUID tables, or the `tests/common/servicing_dependency_fixture_builder.py` helper; on every CI run | No  |
| `servicing_dependency_layer1_test.py` (T13) | Offline self-verification of the Phase 2b2/2b3 Layer 1 writeback helper `Update-Layer1DependencyVerification`. Drives the parser against the T12 fixture, then calls the helper against a tempdir-cloned `data/config-Server*.json` skeleton. 15 assertions covering: stub-config setup pre-flight, Run-1 counts (`UpdatedCount=2`, `UnchangedCount=0`, `MissingCount=2`), field-level correctness of `_DependencyVerifiedUpdateId` / `_DependencyVerifiedRevisionId` / `_DependencyVerifiedCreationDate` on Server 2022 and Server 2025, missing-OS hygiene (no spurious writeback for Server 2016/2019), existing-field preservation (`OsKey`), and idempotent Run-2 (`UpdatedCount=0`, `UnchangedCount=2`). The test never touches the repository's real `data/`. | After every change to `Update-Layer1DependencyVerification`, the `$Script:OfflineSyncOsCategoryGuids` table, or the A04 wrapper's Layer 1 callout site; on every CI run | No  |
| `servicing_dependency_deny_list_test.py` (T14) | Offline self-verification of the EOS/ESU deny-list branch of the Stage 3 scope filter. Drives `ConvertFrom-OfflineSyncPackage` against `fixtures/servicing-dependency/deny-list-package.xml` (deny-only 2012 R2, overlap 2012 R2 + 2016, deny-only 2008, and an unknown-product bundle). 10 assertions covering: `Stats.eosEsuBundlesExcluded` / `eosEsuFamiliesExcluded` accounting, admit of the overlap bundle (allow-overrides; the deny-overrides trap), exclusion of the deny-only bundles, and that an unknown-product bundle is excluded without being counted as EOS/ESU. Executable check that the PowerShell filter matches the `classify_scope` reference. SPEC §B.19.4.1. | After every change to the Stage 3 scope filter, the allow/deny GUID tables, or the deny-exclusion accounting; on every CI run | No  |
| `servicing_dependency_servicing_stack_test.py` (T15) | Offline self-verification of the M1 servicing-stack extraction functions `Resolve-OfflineSyncRevisionToCab` and `Get-OfflineSyncServicingStackInfo`. Drives both against a trimmed `index.xml` CABLIST and three minimised real-cab CBS leaf metadata fixtures under `fixtures/servicing-dependency/cbs/`. 16 assertions covering: RANGESTART revision->cab mapping (exact-boundary, one-below-boundary, and zero cases), and servicingStackModel / requiredServicingStackVersion derivation for separate (2016, real `installerAssembly` build), combined (2022, `6.0.0.0` nominal + inline `Package_for_ServicingStack_<nnnn>`), and checkpoint (2025, no CBS metadata), plus empty-input robustness. The functions take text input, so no cab download or 7-Zip run is needed. SPEC §B.19.9. | After every change to `Resolve-OfflineSyncRevisionToCab`, `Get-OfflineSyncServicingStackInfo`, or the servicing-stack model derivation; on every CI run | No  |
| `servicing_dependency_readiness_verdict_test.py` (T16) | Offline self-verification of the Phase 2c readiness verdict `Test-PatchServicingReadinessFromGraph`. Drives the function against `fixtures/servicing-dependency/readiness-database.json` (separate/combined/checkpoint updates, an in-scope-superseded update, and an out-of-scope-superseded update). 21 assertions covering: presence (`NotInDatabase`), SS-version comparison for separate OSes (`SsTooOld` = 0x800f0823 when provided < required; `Pass` when provided >= required via WimMountState or PolicyOverride), N/A skip for combined/checkpoint, supersession only when the successor revision is itself in scope, verdict precedence, `OverallStatus` roll-up, ISO-8601 `generatedAt` normalisation, and `Available`/`Unknown` on a missing database. The function's only I/O is reading the Layer 2 JSON. SPEC §B.19.10. | After every change to `Test-PatchServicingReadinessFromGraph` or the verdict model; on every CI run | No  |
| `servicing_dependency_recency_fallback_test.py` (T17) | Offline self-verification of the recency fallback in `Test-PatchServicingReadinessFromGraph`. Drives the function against `fixtures/servicing-dependency/recency-fallback-database.json` (two Server 2016 LCUs of different dates + one Server 2022 LCU). 15 assertions covering: an out-of-scope KB falling back to the newest in-scope LCU for its OS family -> `Superseded` (with the fallback target's `UpdateId` / required SS / model surfaced and the target KB named in `Notes`), family resolution from an exact or free-form `OsKey`, `NotInDatabase` when the family cannot be resolved or has no in-scope LCU, direct match taking precedence over fallback, and the `OverallStatus` roll-up. The function's only I/O is reading the Layer 2 JSON. SPEC §B.19.10. | After every change to the recency-fallback logic, the OS-family resolution, or the newest-in-scope-LCU index; on every CI run | No  |
| `servicing_dependency_servicing_stack_populate_test.py` (T18) | Offline self-verification of the I/O-free halves of the servicing-stack populate. Drives `Select-OfflineSyncLcuLeafRevision` (leaf choice: single / closest-below-bundle / greatest / empty) and `Update-ServicingStackFromMeta` (populate `requiredServicingStackVersion` / `servicingStackModel` from CBS metadata, null `providedServicingStackVersion`, skip when leaf meta absent) against the `fixtures/servicing-dependency/cbs/` leaves. 17 assertions. The 7-Zip wrapper `Invoke-OfflineSyncLeafServicingStackExtract` is NOT exercised here (live-CI only). SPEC §B.19.9. | After every change to the SS populate functions or the leaf-selection heuristic; on every CI run | No  |
| `servicing_dependency_data_contract_test.py` (T19) | Offline self-verification of `Test-DataContractConsistency` (wired into A04 in r11.10). Writes synthetic artifacts into a tempdir (never touches real `data/`) covering each status — Current / Stale (older or unstamped) / Refuse (newer) / Foreign (other id family) / Unknown (no `_meta`) — and asserts the per-artifact classification, the directory-argument expansion to `*.json`, the worst-status roll-up (Unknown never worsens), and that the committed `data/servicing-dependency-database.json` classifies Current. 11 assertions. SPEC §B.19.10. | After every change to `Test-DataContractConsistency`, the data-contract identity, or the A04 wiring; on every CI run | No  |
...
| `servicing_dependency_scope_invariants_test.py` (scope-invariants gate) | Offline, stdlib-only, no-PowerShell data-driven contract check of the EOS/ESU exclusion model over `data/servicing-dependency-database.json` (production) and `fixtures/servicing-dependency/expected-output.json` (fixture), plus a unit test of the allow-overrides `classify_scope` reference function on synthetic cases. 23 assertions across: GUID-table self-consistency (allow/deny disjoint, canonical case), allow-overrides classification (in-scope LCU, ESU-only excluded, multi-OS overlap KB890830 admitted), scope.productGuids excludes the deny-list, no in-scope update is deny-only, no in-scope payload references an ESU/EOS KB, and the per-OS recency floor. Encodes SPEC §B.19.4/§B.19.4.1 (allow-list + EOS/ESU deny-list, allow-overrides) and the research §5.8/§5.9 findings. The PowerShell deny-list filter matches the `classify_scope` reference. No T number — invariants gate, mirroring the format/schema-gate convention. | On every commit that touches `data/servicing-dependency-database.json`, the scope GUID tables, or the deny-list; on every CI run | No  |
| `servicing_dependency_layer2_schema_test.py` (Layer 2 schema gate) | Offline, stdlib-only validation of the committed `data/servicing-dependency-database.json` against `schema/servicing-dependency-database.schema.json`, reusing the draft-07-subset validator from `config_schema_test.py`. 16 assertions across: full schema conformance, the shared data-contract identity (`dataContractId`/`dataContractVersion`), portable provenance (`sourceCab.sourceUrl`, no filesystem path; `scope.evaluatedAt`, no legacy `now`), the kbIds populate (present on every update, bare numeric strings), the Microsoft-prose hard rule (SPEC §B.19.8), and forward-compatible servicing-stack field shapes. No T number — data gate, mirroring the format/config-schema/scope-invariants gates. | On every commit that touches `data/servicing-dependency-database.json` or `schema/servicing-dependency-database.schema.json`; on every CI run | No  |
...
python3 servicing_dependency_parser_test.py            # T12: 22 wsusscn2 parser pipeline assertions
python3 servicing_dependency_layer1_test.py            # T13: 15 Layer 1 writeback helper assertions
python3 servicing_dependency_deny_list_test.py         # T14: 10 EOS/ESU deny-list warned-exclusion assertions
python3 servicing_dependency_servicing_stack_test.py   # T15: 16 servicing-stack extraction assertions
python3 servicing_dependency_readiness_verdict_test.py # T16: 21 Phase 2c readiness verdict assertions
...
python3 servicing_dependency_layer2_schema_test.py     # Layer 2 schema gate: data/servicing-dependency-database.json vs schema (16 assertions)
...
tests/
  ...
  servicing_dependency_parser_test.py    T12 (Stage 3 + Stage 4 self-verification, 22 assertions)
  servicing_dependency_layer1_test.py    T13 (Phase 2b2/2b3 Layer 1 writeback helper, 15 assertions)
  servicing_dependency_deny_list_test.py T14 (EOS/ESU deny-list warned-exclusion, allow-overrides, 10 assertions)
  servicing_dependency_servicing_stack_test.py T15 (servicing-stack extraction: RANGESTART map + SS-model derivation, 16 assertions)
  servicing_dependency_readiness_verdict_test.py T16 (Phase 2c readiness verdict: presence/SS-compare/supersession, 21 assertions)
  ...
  fixtures/
    ...
    wsusscn2/
      ...
      deny-list-package.xml          # deny-only/overlap/unknown bundles for T14
      cbs/                           # minimised real-cab CBS leaf metadata for T15
        leaf-2016-separate.xml       #   separate model, requiredSs 10.0.14393.7692
        leaf-2022-combined.xml       #   combined model, 6.0.0.0 + inline ServicingStack

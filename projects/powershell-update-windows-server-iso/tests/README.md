...
| `setup_du_discriminator_test.py` (T30) | **SUPERSEDED-PENDING (the declared red on the integration branch, 6/8)**: `Select-SetupDuCandidate` against verbatim live-Catalog rows. The r12 series closed with the discovery model it awaited in place -- declared in `DiscoveryPolicy.SearchProfiles` (T46) with behavioral Catalog coverage in T50 -- while the terminal retained the title-heuristic selector this test pins; the final supersession disposition (revise or retire) is adjudicated in the consolidation stream; 8 assertions | At the consolidation-stream T30 adjudication | No |

...
| `lcu_target_verify_test.py` (T31) | `TargetBuildAfterUpdate` derived-field contract: per-OS evidence comparator (since r11.59: Server2016 KB-id/build fallback; RollupFix OSes by measured build, no-TBAU = INDETERMINATE Warn; since r11.66: Server2016 `KbIdsAtBuild` membership match), committed-data consistency, single-writer wiring, P11 hard-Fail row; 27 assertions | After every change to the TBAU derivation, the comparator, or P11 | No |
...
tests/
  ...
  lcu_target_verify_test.py              T31 (TargetBuildAfterUpdate contract, 27)
  ...
  per_os_evidence_test.py                T37 (per-OS LCU evidence engine, 17)

...
| `setup_du_discriminator_test.py` (T30) | **SUPERSEDED-PENDING at r12.00 (the declared red on the integration branch)**: `Select-SetupDuCandidate` against verbatim live-Catalog rows; the r12 discovery model supersedes title heuristics via `DiscoveryPolicy.SearchProfiles`, arriving progressively (r12.19, r12.51-53) -- the replacement contract is scheduled there; 8 assertions | Re-examine at the r12.19 / r12.51 merge cards | No |
...
| `setup_binaries_sync_test.py` (T40) | Setup-binary sync contract (P08S + P11): plan build gates (setuphost.exe joins at 26100+ only; unknown build degrades to setup.exe with a stated reason), `Get-SetupBinaryFileEvidence` exact size/SHA-256/ISO-8601-UTC measurement + missing-path shape, closed record vocabulary, and structure pins -- P08S registered between P08 and P09 in all three pipeline lists, SHA-verified copy (hard fail), ReadOnly clearing, CSV/JSON evidence artifacts, console before/after lines, boot.wim-side stash + P09 post-overlay reapply, P11 `SetupBinarySync_*` Fail grading, and the release pin (advanced in every merge commit); 16 assertions | After every change to P08S, the Setup-binary plan/evidence helpers, the P09 overlay, or the P11 SetupBinarySync rows | Yes (pwsh REPL) |
...
| `servicing_contract_baseline_test.py` (T45) | Declaration-derived series instrument: validates `data/servicing-contract-baselines.json` (per-OS contract revisions + SHA-256 pins). Its anchor is introduced at r12.44 -- before that the test reports **NOT-YET** (a recorded gap, not a failure) and exits 0 | From r12.44 onward; on every CI run | No |
| `discovery_policy_declaration_test.py` (T46) | Declaration-derived (r12.00): `DiscoveryPolicy.CatalogAliases` + per-Kind `SearchProfiles` are well-formed and internally consistent (query strategies, title accept/reject constraints, classification requirements). Supersedes T28 (T30's behavioural replacement is scheduled at r12.19/r12.51); 112 assertions | After every change to `DiscoveryPolicy` | No |
...
tests/
  ...
  setup_du_discriminator_test.py         T30 (SetupDU title discriminator; SUPERSEDED-PENDING at r12.00, 8)
  ...
  setup_binaries_sync_test.py            T40 (P08S setup-binary sync, 16)
  apply_plan_conformance_test.py         T41 (declared ApplyPlans conformance, 143 @ r12.00)
  servicing_model_declaration_test.py    T42 (SourcePrerequisites/ValidationPolicy declaration, 30 @ r12.00)
  line_integrity_declaration_test.py     T43 (Lines[].Integrity + Roles declaration, 158 @ r12.00)
  compatibility_declaration_test.py      T44 (legacy/canonical meta-contract, 64)
  servicing_contract_baseline_test.py    T45 (servicing-contract baselines; NOT-YET before r12.44)

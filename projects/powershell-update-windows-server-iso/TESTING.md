...
| T24 dism_cleanup_args_test.py (4 assertions, `Get-DismCleanupArgumentList`: default three-token `/Cleanup-Image /StartComponentCleanup` vector with no `/ResetBase`, `-IncludeResetBase` appends `/ResetBase`; guards the comma/`+` precedence collapse behind exit 1639) | ✓ all pass | r11.24 p07-resetbase-off-export-compress build / 2026-06-11 |
| T25 dism_export_args_test.py (4 assertions, `Get-DismExportArgumentList` returns the five-token `/Export-Image ... /Compress:max` vector targeting the requested source index; guards the same precedence trap) | ✓ all pass | r11.24 p07-resetbase-off-export-compress build / 2026-06-11 |
...
python3 tests/dism_cleanup_args_test.py                          # T24: 4 cleanup-arg-vector assertions (1639 collapse guard + /ResetBase opt-in default)
python3 tests/dism_export_args_test.py                           # T25: 4 export-arg-vector assertions (Export-Image /Compress:max)

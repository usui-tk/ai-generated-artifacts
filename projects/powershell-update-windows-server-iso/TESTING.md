...
| T56 research_reference_drift_test.py (32 assertions, the research-reference drift guard from the 2026-08 alignment remediation: scans the current normative files -- SPEC, README/README.ja, TESTING, tests/README and the main script including comments -- and rejects the six statement families the remediation retired: Server 2016 Setup-DU non-existence claims; per-OS-generation Kind-absence claims and the retired Forbidden-Kind-axis phrasing of the P06 check (English and Japanese); stale research-report paths missing the `documents/` prefix; Digest-as-universal-key claims; universal boot.wim LCU-impossibility claims (the bare impossibility phrase counts only with boot.wim in the nearby context, because the script uses it legitimately about derived-value staleness); and PCA2023 boot-manager claims using the retired signing verb (English and Japanese). CHANGELOG stays out of scope; sections marked Historical/Superseded by heading, Status marker or paragraph label are excluded -- history may quote what normative text may not. Every pattern family and exclusion rule is first proven against built-in synthetic samples so the repository scan cannot go vacuous silently. A companion check pins the STAGE 3 trigger surfaces to each other: the workflow `on:` block must be exactly `workflow_dispatch` and §2.5's Current-trigger paragraph must state the dispatch-only status, so either surface changing alone fails the pin. The guard does not turn the research report into executable policy; it only blocks the reintroduction of known superseded statements) | ✓ all pass | research-alignment Phase 3 / 2026-08-09 |
...
python3 tests/research_reference_drift_test.py  # T56: 32 research-reference drift-guard assertions (pure text scan)

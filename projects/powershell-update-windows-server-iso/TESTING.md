...
| T31 lcu_target_verify_test.py (26 assertions, TargetBuildAfterUpdate derived-field contract: per-OS `Test-LcuTargetApplied` comparator behavior (r11.59: Server2016 KB-id hit + build fallback; RollupFix OSes by measured build vs TBAU, below-floor hard Fail, missing TBAU = INDETERMINATE Warn; measured build in Notes), committed configs have `TargetBuildAfterUpdate == <LCU Line>.InScope.build` with no retired fields, seed envelopes carry only reader-backed members (`Schema`+`ChecksumAlgorithm`, plus the optional `BridgeLcu`), both schemas reconciled, and static wiring -- BOTH Lines writers derive TBAU via the single pure `Get-TargetBuildFromLines`, P11 calls the comparator with the per-OS signature as a hard-Fail row, and no code line touches `VerificationMethod`/`ExcludeKbList`) | ✓ all pass | r11.60 kb-alias / 2026-07-07 |
...
| T37 per_os_evidence_test.py (16 assertions, r11.58 per-OS LCU evidence engine: `ConvertTo-TwoPartBuild`; the four forked resolvers (2016 KB-named, 2019/2022/2025 RollupFix-named, 2025 checkpoint highest-build-wins) against real package-name shapes; MS-verified 2024-4B floor boundaries 14393.6897 / 17763.5696 / 20348.2402 / 26100.1; 3-source consensus (registry preferred, agreement needs >= 2 sources); dispatcher throws on unknown OsKey) | ✓ all pass | r11.60 kb-alias / 2026-07-07 |
| T38 media_inspection_test.py (24 assertions, r11.59/r11.60 media-inspection contract: `ConvertFrom-InspectionBuildValue` shape matrix; `Compare-MediaInspection` pure diff (build advance, package delta, prereq flip, boot `_EX` appearance, post-missing, SHA change); `Get-InspectionCrossChecks` observe-first matrix; `Get-KbAliasFromPatchPath` Catalog parent/child KB alias + DotNet-only divergence audit over all four configs; structure pins (P06 pre / P11 SHA identity + post + alias wiring / P13 diff; the invalid `Get-WindowsPackage -ImagePath` path gone)) | ✓ all pass | r11.60 kb-alias / 2026-07-07 |
...
python3 tests/lcu_target_verify_test.py                          # T31: 26 TargetBuildAfterUpdate derived-field assertions (per-OS comparator + derivation helper + data/schema contract + P11 wiring)
...
python3 tests/per_os_evidence_test.py                            # T37: 16 per-OS LCU evidence assertions (forked resolvers; MS-verified 2024-4B floors; 3-source consensus)
python3 tests/media_inspection_test.py                           # T38: 24 media-inspection assertions (pure diff; observe-first matrix; Catalog child-KB alias + DotNet-only audit)

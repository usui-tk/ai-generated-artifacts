...
5. **Self-verification tool suite** — T1 through T57 (canonical inventory in [`tests/README.md`](./tests/README.md); since r12.00 this includes the declaration-derived T41 – T46 set and eight retirements recorded in SPEC §B.15.4; since the series-end test re-implementation campaign it includes the T47 – T52 contracts re-authored from the external implementation's terminal regression set; T53 is reserved for the contract-surface sync test still under adjudication, and T54 – T55 hold the source-file format contract and the analyzer debt-baseline gate, and T56 holds the research-reference drift guard from the 2026-08 alignment remediation, and T57 pins the A00 fail-closed guard placement)
...
| T56 research_reference_drift_test.py (51 assertions, the research-reference drift guard, v2 after the 2026-08-09 re-audit: scans the current normative files -- SPEC, README/README.ja, TESTING, tests/README and the main script including comments -- as normalized logical blocks (Markdown paragraphs and PowerShell comment blocks joined, code markup stripped, whitespace collapsed before matching; the measured v1 miss was a stale norm wrapped across a line break with backticks between its words). Rejects eight retired statement families: Server 2016 Setup-DU non-existence; per-OS-generation Kind-absence and the retired Forbidden-Kind-axis phrasing of the P06 check (English and Japanese); stale research-report paths missing the `documents/` prefix; Digest-as-universal-key; universal boot.wim LCU-impossibility, extended after the third audit with the retired nonservicing formulations -- release-specific measured statements and explicit contract-routing statements stay expressible, pinned by built-in allowed samples (bare impossibility phrasing counts only with boot.wim in the block); PCA2023 boot-manager claims using the retired signing verb (English and Japanese); universal WinRE no-LCU phrasing (armed only with WinRE in the block); and universal digest-mandatory phrasing superseded by the state-driven B.19.2 table. A retired-CLI-alias check rejects the two retired invocation aliases of the public OsVersion/OsLanguage parameters across the Markdown normative files. CHANGELOG stays out of scope; Historical/Superseded regions are excluded -- history may quote what normative text may not. Every family, the alias check and every exclusion rule is first proven against built-in synthetic samples, including wrapped-phrase and markup-split positives. A companion check pins the STAGE 3 trigger surfaces to each other: workflow `on:` exactly `workflow_dispatch` and the Current-trigger paragraph in this file stating dispatch-only. The guard does not turn the research report into executable policy) | ✓ all pass | re-audit remediation / 2026-08-09 |
| T57 a00_rebuild_guard_contract_test.py (10 assertions, the A00 fail-closed guard placement contract -- fourth audit N4-01 mitigation, r12.90: inside the A00 orchestrator body the schema-currency guard must exist, anchor the canonical current version and precede the Stage 2 config write, and the four committed seeds must still declare the legacy 3.0 shape (the guard's precondition). The designed failure of the precondition pin is the signal to revise this contract together with the guard's retirement when the v4 seed migration lands; see §7.0a for the open defect record) | ✓ all pass | fourth-audit remediation / 2026-08-09 |
...
python3 tests/research_reference_drift_test.py  # T56: 51 research-reference drift-guard v2 assertions (pure text scan)
python3 tests/a00_rebuild_guard_contract_test.py  # T57: 10 A00 fail-closed guard placement assertions (pure text scan)

# Schema / format gates (every commit that touches data)
python3 tests/config_schema_test.py          # config schema gate (28; declaration-based v3/v4 selection + currentness/policy pins)

...
└── tests/                            # 自己検証スイート（疎番号 T1-T57 + ゲート。T56 = 研究参照ドリフトガード、T57 = A00 ガードピン）
```
...
| RefreshAllBaselines 後に解決済みセットが OS サービシングモデルに違反 | `Lines[]` に宣言された `PatchModel` の必須 Kind が欠けている、またはいずれかの行が state-driven 整合性ルールに違反 | P06 `Test-PatchModelConsistency` が不足している必須 Kind を挙げて throw します（このチェックが強制するのは必須 Kind と state-driven 整合性のみで、追加の Kind は受理されます）。`PatchModel` が OS と一致するか確認（SPEC.md §B.19 参照）|

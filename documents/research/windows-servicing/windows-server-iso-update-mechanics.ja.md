...
前方互換なインストールメディア（PCA2011 廃止後も起動可能であり続けることを意図したメディア）を生成したいパイプラインは、 PCA2011 が今日まだ受け入れられていても、 PCA2023 信頼チェーンを持つブートバイナリを出荷する必要があります。 これをどう行うかについての Microsoft のリファレンスは `Make2023BootableMedia.ps1` スクリプトで、 Microsoft Support 記事 KB5053484 を介して配信され、 GitHub の `microsoft/secureboot_objects` リポジトリでメンテナンスされています。

ここでバージョン管理上の落とし穴を明確にしておきます:このスクリプトの **内部バージョン文字列は、 内容が変わるリリースを跨いでも「1.4 (2026-03-13)」のまま** です。 リポジトリのリリースタグは進み続けており（本改訂時点の最新署名付きリリースは `v1.6.4-signed`、 コミット `bd7abe3`。 そのスクリプト内容が `main` ブランチと同一であることを 2026-06 に検証済み）、 少なくとも 1 つのリリースは内部文字列を上げずにスクリプトの挙動を変更しています（能動的な `boot.stl` コピーと `oscdimg` 自動ダウンロードが追加された）。 したがって、 このスクリプトの挙動を引用する必要があるドキュメントやツールは、 **リリースタグまたはコミットハッシュ + 関数名** でピン留めすべきであり、 内部バージョン文字列や行番号でピン留めしてはなりません — どちらも、 このスクリプトに対する実際の調査において既に陳腐化を起こしています。
...
- **ビルド時**:`Make2023BootableMedia.ps1` のリファレンスロジック(`Copy-2023BootBins`。 `v1.6.4-signed` のソースに対して直接検証済み)は、 **ステージ済みメディアの `\sources\boot.wim` index 1** を読み取り専用でマウントし、 *そのマウントされた boot.wim 内部の* `\Windows\Boot\` に見つかる `_EX` コンテンツを出力メディアのルート(既存の ISO ルートレベルの `\boot\` および `\efi\` ディレクトリを置き換えまたは補完)へとコピーする。 このとき boot.wim 内に **3 つすべて** のステージングディレクトリ — `EFI_EX`、 `FONTS_EX`、 `DVD_EX` — が存在することを要求し、 1 つでも欠けると「staged but was not updated」と 2024-4B 以降の更新適用を促すメッセージで中断する。 本節の以前の版では「マウントされた *install.wim* からコピーする」と記述していたが、 それは誤りだった — このスクリプトは install.wim を一切開かない。 boot.wim が `_EX` ディレクトリを持たないメディア(Server 2016 / 2019 の通常状態。 boot.wim は LCU では最新化できない。 §3.8 参照)に対する Microsoft の公認ワークアラウンドは、 **サービシング済み install.wim** からバイナリを手動で供給すること(例:`\Windows\Boot\EFI_EX\bootmgfw_EX.efi` をメディアの `\efi\boot\bootx64.efi` へ上書きコピー)である
...
|---|:-:|:-:|:-:|---|
| Server 2016 EVAL ja-jp | ✓ | ✗ | 3 | `EFI_EX` は最近の LCU を install.wim に適用することでビルド時に **合成** する必要がある。 LCU は WIM 内部の `\Windows\Boot\` 直下に `_EX` ステージングディレクトリを直接実体化する |
...
**この合成は OS バージョン間で対称ではありません。** LCU サービシング済み(2026-06 パッチレベル)の install.wim を直接検査すると、 各 OS が実際に何を実体化するかが分かります:

| OS(サービシング済み install.wim） | EFI_EX | FONTS_EX | DVD_EX(`efisys_EX.bin`） |
|---|:-:|:-:|:-:|
| Server 2016 | ✓(LCU 配送) | ✓ | **✗ — 実体化しない** |
| Server 2019 | ✓(LCU 配送) | ✓ | ✓ |
| Server 2025 | ✓(GA 時点で同梱) | ✓ | ✓ |

Server 2016 と 2019 は、 それぞれの LCU から **バイト一致** の PCA2023 署名済み `bootmgfw_EX.efi` / `bootmgr_EX.efi` バイナリを受け取ります。 しかし Server 2016 の LCU は `DVD_EX` ツリーを配送しないため、 `efisys_EX.bin` — Microsoft の 5 ターゲットメディア仕様の *required* ターゲットであり、 `Copy-2023BootBins` が要求する 3 ディレクトリの 1 つ(§3.2) — は、 フルサービシング後でさえ Server 2016 自身のメディア内に供給源を持ちません。 その正規の代替供給源は未解決の問題です(§9):候補は、 兄弟 SKU のサービシング済み install.wim(ブートマネージャーがバイト一致である事実から妥当性は高いが未検証)、 および `oscdimg` と並んで `etfsboot.com` / `efisys` ブートセクタ成果物を出荷する Windows ADK 系統です。
...
Microsoft の `Make2023BootableMedia.ps1` 自体は **署名検証を一切行いません** — ブートバイナリの取り扱いは純粋にファイルコピー操作であり、 これは `v1.6.4-signed` まで変わっていません(後続リリースは能動的な `boot.stl` コピーと `oscdimg` 自動ダウンロードを追加しましたが、 Authenticode 関連コードは依然ありません)。 Microsoft の設計上、 出力検証は呼び出し側の責任です。
...
`\sources\boot.wim` 行についての注意:この行は、 boot.wim が既に最新のブートバイナリを持つメディア(例:Server 2025 GA メディア)を記述しています。 Server 2016 / 2019 では LCU は boot.wim をその状態に **できません**(§3.8) — オフラインサービシングパイプラインは boot.wim をソースとバイト一致のまま残します — ので、 これらの OS では出力メディア上の PCA2023 バイナリは代わりにサービシング済み install.wim から供給され、 boot.wim 内部のバイナリはソースの署名レベルのままとなります。 検証関数は、 boot.wim への期待を普遍的なものではなく OS 条件付きとして扱うべきです。
...
### 3.8 boot.wim は LCU でサービシングされない:WinPE 制約

実際のパイプライン設計を誤らせてきた前提 — 本記事が統合したパイプラインの過去の版も含む — は、 `\sources\boot.wim` を install.wim と同じ方法で当月パッチレベルへ引き上げられる(マウントし、 LCU を適用し、 コミットする)というものです。 **この前提は Microsoft 自身のサービシングモデルに照らして誤りであり、 失敗モードは再現可能です。**

文書化された側面:Microsoft Learn(「Add an update to Windows PE」)は、 WinPE は OS の縮小サブセットであるため、 ほとんどの Windows 更新パッケージは Windows PE に適用されないと述べています。 WinPE は Windows 10 1809 以降、 ADK とは別のアドオンとして出荷され、 **独自のサービシングライフサイクル** を持ちます — OS の LCU ではなく、 専用の更新を受け取ります。

経験的な側面(Server 2019 boot.wim index 1、 2026-06 パッチセット;手法のバリエーションを跨いで再現):

- combined LCU **`.msu`** を `Add-WindowsPackage` で適用すると **`0x80070032` ERROR_NOT_SUPPORTED** で失敗します — WinPE は `.msu` の Unattend 系ハンドラーを即座に拒否します。
- `.msu` を展開し(`expand -f:*`)、 ネストされた CAB を順に適用するとより先へ進みます — **SSU `.cab` はクリーンに適用** されます — が、 **LCU `.cab` が `0x8007371b` ERROR_SXS_TRANSACTION_CLOSURE_INCOMPLETE で失敗** します:LCU の changelist は `BootEnvironment-Core-BootManager-EFI.Resources` / `-PXE.Resources` コンポーネントファミリをロケールごとに pre-stage し、 そこには縮小済み WinPE イメージに端的に存在しない `qps-ploc` / `qps-plocm` 疑似ロケールメンバーが含まれるため、 CBS トランザクションのクロージャは決して完了できません。
- この失敗は構造的なものであり、 修復可能な破損ではありません:SSU と LCU の間にコミット境界を挿入(dismount-save、 remount)しても結果は変わらず、 DISM の修復経路も閉じています — `/Cleanup-Image /ScanHealth` と `/RestoreHealth` は WinPE ターゲットを拒否する(「Scan health cannot be run on a Windows PE target image ... full Windows OS image only」)ため、 欠落メンバーをオンライン・オフラインいずれの修復ソースからも供給できません。

ここから 2 つの設計上の帰結が導かれます。 第一に、 パイプラインは「boot.wim に LCU を当てる」ステップを計画してはなりません;LCU のブートバイナリペイロード(PCA2023 変換が必要とするまさにその `EFI_EX` コンテンツ)は **サービシング済み install.wim** にのみ実体化し、 変換はそこから供給しなければなりません — §3.2 で述べた Microsoft の公認ワークアラウンドそのものです。 第二に、 この制約は *パッケージサービシング* に対するものであり、 ファイルシステムに対するものではありません:boot.wim を読み書きでマウントし **ファイルをコピーで持ち込む** こと(例:`Make2023BootableMedia.ps1` の boot.wim 内部への期待を満たすために `_EX` ディレクトリをステージングする)は、 通常のマウント/コミット衛生の下で機構的に可能なままです。 WinPE が拒否するのは CBS パッケージトランザクションであって、 書き込みではありません。

(補足:install.wim 内の WinRE イメージも LCU ではサービシングされません;Microsoft の media-dynamic-update シーケンスは代わりに Safe OS Dynamic Update でサービシングします。 その経路はこの制約の影響を受けません。)
...
2. **サービス スタック バージョン比較** -- LCU が実在の `installerAssembly` 下限値を宣言する OS(2016、 2019)について、 下限値が満たされることを検証する。 Server 2016 では同じパッチセット内のスタンドアロン SSU がその下限以上のサービス スタック バージョンを提供しなければならない。 Server 2019 はペアにすべきスタンドアロン SSU が存在しないため、 下限値は別パッケージではなく base イメージ上(または LCU 内包)に既に存在するサービシングスタックに対して検証する。これが実際に `0x800f0823` を防ぐチェックである。
...
### 5.10 イメージ側 servicing-stack フロア:メタデータが運ばない第三の軸

§5.2〜§5.4 は SSU 問題の 2 つの軸 — *standalone SSU が当月公開されているか*(Catalog 検索の答え)と、 *LCU が宣言する最小 servicing-stack バージョンは何か*(CBS `installerAssembly` フロア) — をカバーしています。 2026 年の Server 2022 での実インシデントが、 **そのどちらも捉えない第三の軸** を露呈させました。 これは Combined モデルをオフライン適用する場合に固有のものです:

- 評価版メディアの Server 2022 install.wim(servicing stack `10.0.20348.557`、 GA 世代)を 2026-05 の combined LCU(KB5087545、 `IsCombined`)でオフラインサービシングしたところ、 CBS はイメージの古い servicing stack をロードした後、 **LCU 自身に埋め込まれた `SSU-20348.5120` CAB の展開に失敗**(DPX `TOC.xml` エラー)し、 **`0x800f0823 CBS_E_NEW_SERVICING_STACK_REQUIRED`** を返しました — 古いスタックは、 自身の後継を運ぶパッケージフォーマットを文字通り展開できないのです。
- この要件は Microsoft によって文書化されています — ただし **LCU の KB ページ上のプロズ(散文)としてのみ**:オフラインメディアの場合、 イメージは **LCU KB5030216(2023-09-12)以降** を既に含んでいなければならず、 それが servicing stack を文書化された最小値 **20348.1960** へ引き上げます。 したがって Server 2022 でのブートストラップの救済策は *過去の LCU* であり、 standalone SSU ではありません(2022 には公開されていません)。
- 決定的なのは、 このフロアが **いかなる機械可読サーフェスにも存在しない** ことです:2022 LCU の `installerAssembly` は名目上のプレースホルダ `6.0.0.0`(§5.4)なので、 そこから導出される「必要 servicing-stack バージョン」は空であり、 `wsusscn2.cab` にもこれを表現するものは他にありません。 メタデータ駆動の事前検証ゲート(§5.5)は **このフロアを導出できません**;パッチ *セット* のベースライン事前検証は、 そのセットが適用される *イメージ* について何も語らないのです。

したがって実用的な緩和策は、 それぞれの側が知り得るものによって分割されます:

1. **データ側**:OS ごとの、 オペレータがキュレートする `最小イメージ servicing-stack` フロア。 KB ページのプロズから取得し、 **引用とともに** 記録します(KB 番号と、 値の出典となった文)。 これにより値の出所がレビューに耐えます。 この値が今日機械導出できないという事実に対して誠実な方法です。
2. **イメージ側**:マウント時に、 マウントされたイメージの実際の servicing-stack バージョン(`Microsoft-Windows-ServicingStack` コンポーネント / WinSxS ディレクトリのバージョン)を読み取り、 キュレートされたフロアと比較します — *フロア不明* は警告に降格し、 *フロア既知かつ未達* は高コストな WIM 作業の前に実行を停止します。 §5.5 の fail-cheap 哲学を反映したものです。
...
どちらの失敗も慣習的な意味で「スクリプトの責任」ではありません;それらは、 データが許す限り事前検証で表面化させるべき前提条件の問題を反映しています。 その「データが許す限り」を画定する文書化済みの例外が 1 つあります:combined モデルの **イメージ側 servicing-stack フロア**(§5.10)です。 この要件は KB ページのプロズとしてのみ存在するため、 メタデータ駆動のいかなる事前検証からも不可視であり — このクラスには config 検証の改善ではなく、 マウント時のイメージ側バージョンチェックが必要です。 いずれの場合も、 修復は声を大にして失敗し、 WIM をクリーンな状態でマウント解除し、 オペレータにどの前提条件が注意を要するか伝えることです。
...
5. **Server 2025 DU.Setup ケイデンス**:セクション 6.3 で述べたように、 Microsoft はケイデンスを正式にアナウンスしていません。 2025-12 から 2026-04 のギャップの後に 2026-06 の公開(KB5095966)が続いたため、 パターンは打ち切りではなく断続的である;意図的な不規則 / 四半期ケイデンスへの移行か、 単に偶発的なギャップかは未確認のままです。
...
7. **Server 2016 の `DVD_EX` / `efisys_EX.bin` の正規代替供給源**:Server 2016 の LCU は `DVD_EX` ツリーを install.wim に配送しません(§3.3)が、 `efisys_EX.bin` は Microsoft の 5 ターゲットメディア仕様の required ターゲットであり、 `Make2023BootableMedia.ps1` が要求する 3 ディレクトリの 1 つです(§3.2)。 候補となる供給源は 2 つあり、 どちらも未検証です:**兄弟 SKU のサービシング済み install.wim**(Server 2016 と 2019 はそれぞれの LCU からバイト一致の PCA2023 署名済みブートマネージャーを受け取るため、 `DVD_EX` 資産のクロス SKU 互換性は妥当性が高いものの未証明)、 および **Windows ADK 系統**(`etfsboot.com` / `efisys` ブートセクタ成果物が `oscdimg` と並んで出荷されます。 現行 ADK が `_EX` 相当を持つか、 非 `_EX` の `efisys` が PCA2023 強制ファームウェアで受け入れられるかは、 どちらも未検証)。 これの解決が、 外部資産なしで Server 2016 メディアを完全に PCA2023 ブータブルにできるか否かを決定します。
...
- WinPE が一般の Windows 更新パッケージの適用対象でないこと(Microsoft Learn;§3.8 の文書化された根拠)、 および WinPE アドオンが独自のサービシングライフサイクルを持つこと。

...
**公式適用範囲(Microsoft KB 5053484)**: 上流の手順は Windows Server
2012/R2/2016/2019/2022 と Windows 10/11(version 1607 以降)を
公式サポート対象としています。**Server 2025 は意図的に "Applies To"
リストから除外**されています — ファームウェアに 2023 証明書が
事前搭載されているため、本手順は不要です。本プロジェクトは
この公式スコープを踏襲します:Server 2016/2019/2022 は P10 の
デフォルト対象、Server 2025 は明示的な
`-ForcePca2023OnServer2025` オプトインが必要です。クライアント
SKU(Windows 10/11)は本プロジェクトのスコープ外です。

設計上の知見は `SPEC.md` Part D.22、運用モデルは `SPEC.md` B.18、
UEFI セキュアブートのテンプレートプロファイル(ターゲット
ハードウェアのファームウェア設定の選び方)については
`SPEC.md` D.23 を参照してください。

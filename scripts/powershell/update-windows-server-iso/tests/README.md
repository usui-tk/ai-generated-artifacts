...
| `wsusscn2_servicing_stack_populate_test.py` (T18) | Offline self-verification of the I/O-free halves of the servicing-stack populate. Drives `Select-WsusScnLcuLeafRevision` (leaf choice: single / closest-below-bundle / greatest / empty) and `Update-WsusScnServicingStackFromMeta` (populate `requiredServicingStackVersion` / `servicingStackModel` from CBS metadata, null `providedServicingStackVersion`, skip when leaf meta absent) against the `fixtures/wsusscn2/cbs/` leaves. 17 assertions. The 7-Zip wrapper `Invoke-WsusScnLeafServicingStackExtract` is NOT exercised here (live-CI only). SPEC §B.19.13.0. | After every change to the SS populate functions or the leaf-selection heuristic; on every CI run | No  |

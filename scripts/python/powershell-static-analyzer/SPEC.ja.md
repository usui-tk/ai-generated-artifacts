...
- [付録 A — ルール重大度マトリクス](#付録-a--ルール重大度マトリクス)
- [付録 B — ドキュメント履歴](#付録-b--ドキュメント履歴)
- [付録 C — 品質ゲートと検証チェックリスト](#付録-c--品質ゲートと検証チェックリスト)
- [付録 D — 既知の落とし穴と教訓](#付録-d--既知の落とし穴と教訓)
...
## 付録 C — 品質ゲートと検証チェックリスト

> 本付録は、本リポジトリ内の姉妹スクリプト SPEC
> （`ol-aws-ami-builder/SPEC.md`、`download-speakerdeck-oracle4engineer/SPEC.md`）で採用されている
> **Part C** 規約を反映したものです。`psa.py` の本仕様書本体は API 仕様
> （番号付き §1〜§11）として構成されているため、同等の内容は付録として
> ここに配置しています。

`psa.py` への変更コミット前に、以下すべてをパスする必要があります。

### 静的チェック

- [ ] `python3 -m py_compile psa.py` → エラー 0 件（構文のみのチェック）
- [ ] `python3 psa.py --list-rules` が exit 0 でドキュメント記載の全ルールを列挙（`RULES` タプルの内部整合性を確認）
- [ ] 新たな外部依存を導入していない（`psa.py` は §1.3 の通り標準ライブラリのみで動作すること）
- [ ] Python 3.8（§1.3 の最小サポートバージョン）で変更後の `psa.py` が動作する
- [ ] 新規ルールコードは `PSAxxxx` 命名規約（§4）に従っている

### 機能チェック（自己解析）

- [ ] `python3 psa.py psa.py` を実行し、`PSA1xxx`（構文・構造）の検出が 0 件（ツールが自分自身を解析できる）
- [ ] `python3 psa.py --format json psa.py` が `python3 -c "import json,sys; json.load(open('output.json'))"` でパースできる JSON を出力
- [ ] `python3 psa.py --format sarif psa.py` が `github/codeql-action/upload-sarif` で受理される SARIF 2.1.0 ドキュメントを出力
- [ ] インライン抑制指示（`# psa-disable-line`、`# psa-disable-next-line`、`# psa-disable-file`）が対象コードのみを抑制し、他のコードには影響しない（§7）

### コンシューマー回帰チェック

- [ ] `python3 psa.py ../../powershell/download-speakerdeck-oracle4engineer/Download-SpeakerDeck.ps1` が 0 errors / 0 warnings / 0 info を報告（リポジトリ内コンシューマーの定常状態）
- [ ] `python3 psa.py ../../powershell/download-speakerdeck-oracle4engineer/Test-PdfMetadata.ps1` が 0 / 0 / 0
- [ ] これまでクリーンだったスクリプトを新たに検出させ得るルール変更を行う場合、外部コンシューマー（`usui-tk/Deploy-AMD-Drivers-For-WindowsServer`）に通知（README.md の「新規ルール追加」ガイドに従う）

### ドキュメントチェック

- [ ] `README.md` が新規 CLI フラグ・ルールコード・設定フィールドをすべて言及している
- [ ] `README.ja.md` が構造（表のレイアウト・セクション順）で英語版と一致している
- [ ] 新規ルール追加時、`README.md`・`README.ja.md`・本 SPEC §4・付録 A をすべて同時更新している
- [ ] バージョンバンプ（付録 B）が変更カテゴリを反映している: パッチ（バグ修正）、マイナー（新規ルール・新規機能）、メジャー（破壊的変更）
- [ ] `--check-env` / `--show-env` の出力が安定している（CI 連携を壊さない）

### クロスフォーマット / スキーマチェック

- [ ] JSON 出力スキーマ（§6.2） — パッチ・マイナーリリースでフィールド名や型を変更しない
- [ ] SARIF 出力（§6.3） — `tool.driver.version` が `psa.py` の自己報告バージョンと一致する
- [ ] 終了コード（§9） — 同一メジャーバージョン内では `0 / 1 / 2` のセマンティクスを維持する

---

## 付録 D — 既知の落とし穴と教訓

> 各エントリは、実運用で発見された `psa.py` のバグと、その修正および再発防止のための設計ルールを記録するものです。
> 将来の改修でこの修正を継承し、同じバグを再度作り込まないようにします。

### D.1 ヒアドキュメント / サブ式トークンがルールスキャンに漏れる（2.0.0）

**症状**: `PSA2003`（`-match` 対 bare `$variable`）等のルールが、`@"…"@` ヒアストリング内部でも発火し、デモ目的で PowerShell 風構文を含む docstring や `Write-Host` ブロックで誤検出を生じていた。

**根本原因**: 初期の `strip_strings_and_comments()` が PowerShell ヒアストリングを認識しておらず、その内容が正規表現ベースのルールへそのまま渡っていた。

**修正**: トークナイザ（§10）が `@"…"@`、`@'…'@`、`$()`、`@()` 構文の内容を、行番号を維持しつつ空白で埋めて除去するようになった。新規ルールは、明示的に raw 形式を必要としない限り、トークナイズ済みテキストを消費すること。

### D.2 自動変数リストのドリフト（`$using:` の導入）

**症状**: Windows PowerShell 5.1 で `$using:` リモートスコープが導入された後、`PSA2001` が `$using:` 接頭辞付きの変数を未定義として誤検出するようになった。

**根本原因**: `psa.py` の自動変数許可リストに、`$using:` がスコープ接頭辞として登録されていなかった。

**修正**: 自動変数チェック前に、スコープ接頭辞（`$global:`、`$script:`、`$local:`、`$private:`、`$using:`、`$env:`、`$variable:`）を除去する。上流で新しい PowerShell スコープ接頭辞が判明した場合は、このリストに追加し、該当ルールの docstring に検証用 PowerShell スニペットを固定すること。

### D.3 SARIF 出力が GitHub Code Scanning で拒否される（初期 2.0.x）

**症状**: アップロードした SARIF ドキュメントが `The SARIF file contains a Validation Error` で拒否された。

**根本原因**: 初期の SARIF 出力が `tool.driver.rules` 配列を省略していた。SARIF 2.1.0 仕様上はオプションだが、GitHub のバリデータはこれをハードエラーとして扱う。

**修正**: `format_sarif()` は、今回の実行で検出有無に関わらず、既知の全ルールを `rules` 配列として常に出力する。これは恒久的契約であり、最適化のために削除してはならない。

### D.4 `.psa.config.json` が CI の `$HOME` 配下で検出されてしまう

**症状**: CI 実行で、ランナーホームディレクトリにあった古い設定ファイルが拾われ、本来有効化されているべきルールが無効化されることがあった。

**根本原因**: 初期の暗黙検索ロジックが `/` まで上位ディレクトリを遡って探索していたため、`$HOME`（その先祖が `/home/runner`）にある `.psa.config.json` が優先されてしまった。

**修正**: 暗黙検出は、`.psa.config.json` を含む最初の先祖ディレクトリ、または git リポジトリルート（`.git/` を含む先祖）のいずれか先に到達した方で停止する。CI では完全に明示的な設定指定のため `--config <path>` を使用すること。

### D.5 リモート `--config` 取得が CDN のボットフィルタでブロックされる（2.3.0 以前）

**症状**: `--config https://raw.githubusercontent.com/...` が開発者ラップトップでは動作するが、CI では Cloudflare 前段のフォークに対し HTTP 403 または TLS ハンドシェイクエラーで失敗していた。

**根本原因**: `urllib` のデフォルト User-Agent（`Python-urllib/3.x`）は WAF のボットヒューリスティックで既知のパターンであり、一部 CDN のデフォルト設定で即時拒否される。さらに、`urllib` は OS のデフォルト設定次第で TLS 1.0/1.1 をネゴシエートする可能性があり、モダンサーバではこれを拒否する。

**修正**: §5.4 に明記 — 明示的な TLS 1.2 最小バージョン SSL コンテキスト、Chrome 131 User-Agent と Sec-Ch-Ua クライアントヒント、5xx およびネットワークエラーに対する指数バックオフリトライ（4xx はリトライしない）。`PSA_CONFIG_TIMEOUT`、`PSA_CONFIG_MAX_RETRIES`、`PSA_CONFIG_QUIET` でチューニング可能。

### D.6 JSONC のコメント除去が文字列リテラル内に誤適用される

**症状**: `"description": "use // to enable trace"` を含む `.psa.config.json` で、コメント除去パス実行後に JSON パースエラーが発生。

**根本原因**: 初期のコメント除去が文字列境界を考慮していなかった。

**修正**: JSONC ストリッパは、エスケープされたクォートを考慮しつつ文字列リテラル状態を追跡し、文字列リテラル外の `//` および `/* */` のみを除去する。文字列内の `//` は逐語的に保持する。

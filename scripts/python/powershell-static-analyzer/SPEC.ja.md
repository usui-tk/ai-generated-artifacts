...
ルールコードは 2 つの形式で指定できます:

- **新形式**: `PSAxxxx`（例: `PSA2001`）
- **レガシー形式**: `Cn`（例: `C4`）— 大文字小文字非区別

両形式は、コードを期待するすべての箇所（`--enable`, `--disable`,
`--include`, インライン抑制コメント、`.psa.config.json`）で受け付け
られます。レガシーコードは処理前に内部で `PSA` 形式に変換されます。

カンマ区切りリストを 1 つの引数値として渡せます（例:
...
### 4.5 PSA2002 — auto-variable 上書き（レガシー C5）
...
### 4.6 PSA2003 — `-match` を素の変数に対して使用（レガシー C7）
...
### 4.10 PSA3001 — `Start-Process -ArgumentList`（レガシー C6）
...
### 4.11 PSA3002 — バッククォート継続行の次が空行（レガシー C9）
...
### 4.12 PSA3003 — `-match` を空文字列に対して使用（レガシー C10）
...
| フィールド | 型 | 既定値 | 備考 |
|:---|:---|:---|:---|
| `enable` | string 配列 | `[]` | 各文字列はルールコード（`PSAxxxx` またはレガシー `Cn`）。未知のコードは黙って無視 |
...
`psa.py` 1.x のレガシーコード `C1`〜`C10` は、入力形式
（`--enable`, `--disable`, `--include`, 設定ファイル, インライン抑制）
として受け付けられ、内部で `PSA` 形式にマッピングされます:
...
(\*) `C1`, `C2`, `C3` は括弧バランスルールにマッピング。レガシー実装で
は 3 種の括弧を 1 チェックとして扱っていましたが、新実装では分離して
います。括弧／丸括弧／角括弧チェックは 1.x でも既に error 重大度で
あったため、レガシーエイリアス `C1`/`C2`/`C3` は厳密には必須では
ありません。

テキスト出力では、レガシーコードが存在する場合は両形式を出力します
（例: `[PSA2003 (=C7)] line 2337: 22: …`）。CI ログで `C7` を grep
していたツールは引き続き動作します。

### 10.2 終了コードの安定性

`0` / `1` / `2` の終了コード契約は 1.x から保持されています。1.x で
0 errors だったファイルが 2.x で error を返す場合があります（特に
PSA5001 など新セキュリティルールの影響）。これは意図的な変更で、
ルールセットの拡張によります。

### 10.3 設定ファイルの安定性

`psa.py` 2.x で `.psa.config.json` ファイルを追加。それ以前のバージョン
は全ての設定ファイルを無視していたため、本追加は純粋に加算的です。

### 10.4 削除・変更された挙動

なし。`psa.py` 2.x は機能面で 1.x に対し厳密に加算的です。
...
1. `psa.py` 冒頭の `RULES` タプルリストにエントリを追加:
   ...
   5 タプル: `(code, severity, legacy_code_or_None, default_enabled, short_message)`

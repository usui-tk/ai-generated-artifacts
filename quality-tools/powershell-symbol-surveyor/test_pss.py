...
# (`check` -> `cchk`, `git` -> `corpus_git`, `run` -> `corpus_run`).
#
...
def commit(repo, path, body, message):
    ...
    with open(full, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(body)
    corpus_git(repo, "add", "-A")
    corpus_git(repo, "-c", "user.name=T", "-c", "user.email=t@e", "commit",
...
def test_refusals(tmp):
    ...
    cchk("refuse: a non-.ps1 path", rc == corpus.RC_ERROR, out)
    ...
    cchk("refuse: an absolute path", rc == corpus.RC_ERROR, out)
...
def test_rename_boundary(tmp):
    ...
    corpus_git(repo, "mv", "old/S.ps1", "new/S.ps1")
    corpus_git(repo, "-c", "user.name=T", "-c", "user.email=t@e", "commit",
...
def test_growth_and_rewrite(tmp):
    ...
    rc, out = corpus_run(["--root", root, "--repo", repo, "check"])
    cchk("check: clean again after re-pinning", rc == corpus.RC_OK, out)
    ...
    corpus_git(repo, "reset", "--hard", "HEAD~2")

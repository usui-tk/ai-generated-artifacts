...
### 3.3 External adoption evaluations (record)

Distinct from §3.2. A survey round asks a consumer to judge a *design*; an
adoption evaluation is a consumer deciding whether to **use** this tool for
real work, and it returns a different kind of finding — not what the model
should carry, but what happens when a caller relies on it.

**2026-08-23, first adoption evaluation.** An external project evaluated PSS
at `322d85d6` (`model_version` 7, pss 0.7.0) for adoption against its own
first-party analyser, which is built on native `Parser::ParseFile()`. The
method was a differential against that authority over the project's live
baseline, plus an independent replay of the 230-generation cache.

What the differential *confirmed*, exactly: 480 functions with every start
and end line, 1,247 function-pair edges with per-edge call-site counts, 34
direct root callees, 24,317 variable references, 5,074 CommandAst (5,048
named + 26 dynamic), and 3 mutually-recursive components. Both cache entries
replayed with every record's version, axes, path and commit/blob identity
agreeing with the header, and the baseline digest and both model-shape
fingerprints matching this build. **This is the strongest external
verification the tool has received, and it is a verification of the facts
the tool emits.**

The outcome was nonetheless **conditional adoption only** — as supplementary
evidence, explicitly not as the analysis authority, with a contract whose
first clause is *run the native parser first and require zero parse errors*.
That clause is the finding. It is a consumer-side workaround for something
this tool cannot say, and the evaluation named it: given a script with an
unclosed brace, PSS exits 0, reports no functions, and declares no
limitation. Re-measured here and found worse than reported — on a file whose
*middle* function is unterminated, PSS emits the surrounding functions as
ordinary facts and silently drops the broken one, so the model does not
merely stay silent, it states a confident and wrong inventory. §1.3 says an
identity this tool cannot state is omitted, never guessed; a silently
partial inventory presented as a whole one is the same failure wearing the
opposite face.

The evaluation also named boundaries this SPEC already declares (§12.8's
unmodeled channels, cross-file and module scope, runtime binding,
reachability from a dynamically-resolved entry) and correctly concluded PSS
cannot settle them alone. Those are not defects; that they were discoverable
only by reading this document rather than the model is. Four rows enter
...
remain uncommitted. **No echo-back round follows (owner's adjudication,
2026-09-07):** the survey instrument exists to collect *use cases* from
consuming analysers, not to have them review this stream's implementation
or design; carriage of accepted clause texts is verified in-stream (the
committed §12.10 / §12.8 / descriptor texts against the accepted texts,
by the gate that holds them), and the stream decides the specification.
The `hash_alpha` basis question this round surfaced was decided that
way — by measurement and the owner's adjudication (§10.9), not by a
further round.
...
carrying `owner` and `line`. Measured on the reference target at `model_version` 5 and re-verified at 6
(the D14 byte-offset conversion moved span *values*, not sizes: the default
model is byte-identical and the `local-sites` increment moved by 2 bytes —
digit-boundary crossings only; the v5 figures below therefore stand): the
...
default model (the script writes, scope (ii)). D15's own prices, measured
at `model_version` 7 on the reference target: the `resolved-sites` axis
increment is **491,981 B** (+44.1% of the v7 default — paid only when the
axis is requested), and `rhs_refs` (§12.9) adds **26,668 B to the default
model** (the script writes — the same scope-(ii) distribution as D13's
`rhs`) and **691,996 B in total on the default+`local-sites`
materialisation**; both figures re-measured from the landed build, and
within accounting noise of the round-6 prototype figures the adjudication
priced (§3.2). (Both figures are the compact-JSON byte length of
...
- `site_records` — under the `resolved-sites` axis only (§5.6, D15): one
  record per site, in the same ascending order as `lines`, carrying the
  byte span and argument itemisation the fold withholds.
...
### 5.10 The scan-outcome declaration (normative)

Every model carries a top-level **`scan`** block — always present, on every
materialisation, zero-valued on a clean scan:

```json
"scan": {"outcome": "no-anomaly-detected", "checks": 6, "anomalies": 0}
```

A key that appears only on failure cannot be told apart from a tool that
never looked — which is precisely the confusion the §3.3 adoption evaluation
had to work around by running a real parser first. The shape follows the
evaluating analyser's always-present `Parse` block (`ErrorCount` present and
zero on success) rather than a key that appears only on failure. Measured at
the pinned blob the block costs **66 bytes** with its key (0.006% of the
default model).

**The six checks.** PSS has no grammar (§1.1), so what it can honestly
verify is termination and balance over its **own token stream** — the same
stream every fact is derived from, so an anomaly here is an anomaly in
exactly what the model was read through:

| Check | Reports |
|---|---|
| `unterminated-function-body` | a function or filter declaration whose body brace never closes |
| `unterminated-declaration-parameters` | a declaration parameter list whose parenthesis never closes |
| `unterminated-lexical-region` | a quote, here-string or block comment the scan had to truncate at end of input because its terminator never appeared |
...
A1/A2 are wired at the scanner's own two silent-skip sites (`find_functions`
returning past an unmatched brace or parenthesis): the skip stays — there is
no body to survey — and what changed at D16 is that the skip is **reported**.
Until then a file whose middle function was unterminated yielded a model that
stated the *other* functions as an ordinary, complete inventory: not
silence, a confident wrong answer. A3 reads the tokenizer's own
close-at-end-of-input behaviour as the fact it is; A4–A6 walk delimiter
balance over significant tokens — a brace inside a string or comment cannot
derail them (the §10.6 discipline) — and report both an unmatched closing
delimiter (where depth first went negative) and unclosed depth at end of
input. `--self-check` holds this table against `pss.SCAN_CHECKS` in both
directions, on name and description.

**The outcome vocabulary claims no success.** `outcome` is exactly one of
**`no-anomaly-detected`** / **`anomalies-detected`**. Neither means
"parsed": the honest report is what six named checks found, never a claim
about the language. `checks` states the declared check count, so a reader
can tell this build's coverage without the SPEC.

**PSS9001 records.** Each anomaly emits one PSS9001 record into
`limitations` — limitations-shaped plus `check` (which check fired) and,
where the check names a concrete construct, `target` (§4.8). `anomalies`
equals the PSS9001 record count by construction, and the gate holds it.

**No new exit code.** A scan anomaly is a fact about the input, not a
verdict (§1.2, §9): exit 0 either way, and a consumer branches on
`scan.outcome`, not on the process status.

**Measured, not asserted (D16 design measurement, 2026-08-23).** Native
`Parser::ParseFile()` acting as the oracle over 175 systematic mutations of
the pinned basis itself (delimiter deletions, quote deletions, truncation at
random offsets — real 1.45 MB code, not hand-built fixtures): of the 147
mutations native rejects, the six checks catch **146 — 99.3%** — with **zero
false positives** across the 28 mutations native still accepts, across 83
real PowerShell files (3.89 MB), and across **all 230 committed corpus
generations of both entries** (re-measured on the landed build). The single
miss is a **named class, not a bug**: deleting one `"` left the quote count
even, the file re-lexed with shifted string boundaries, every delimiter
still balanced, and native's errors were grammatical. **These checks see
termination and balance; a grammatical error that leaves every delimiter
balanced is structurally invisible to them.** That sentence is the
completeness statement; the percentage is what makes it credible.
Completeness is therefore **not claimable and not claimed** — which is why
the outcome vocabulary contains no success word, and why the one-direction
differential gate (native error>0 ⇒ anomaly>0, `test_pss.py`) checks
exactly the direction declared and no more. The native parser is a
...
principle. Measured at the pinned blob the block costs **512 bytes** with its key
(0.05% of the default model). The key set is held against the
`limitations` record variants in
...
Measured at the pinned blob the block costs **565 bytes** with its key
(0.05% of the default model). `--self-check` holds this table against
`pss.COLLECTION_DECLARATIONS` in
...
| Determinism | **RESOLVED.** `test_pss.py` extracts the pinned blob twice at each materialisation and compares the **serialised bytes**, not the parsed objects: key order is part of what §5.4 promises, and two dicts can compare equal while serialising differently. Re-checked now rather than left as written, because `--cost` re-runs the survey internally to price an absent axis (§3.1), so a default-materialisation model is produced by five extractions rather than one (one per absent axis; four axes since D15) | closed |
| Reachability | no §10.5 unreachable combination is producible over the regression corpus (`PSS9006` count is zero) | **closed at the D16 arc (C2)** — and no longer vacuous: PSS9006 has an emit path (§10.5 fallback), its `surveyed` tally counts every hash-triple read as `examined` (365 on the adjudicated pair, `emitted: 0`), the reachability gate asserts its absence from every healthy artifact per run, and the fallback itself is fired synthetically through the real compare path (tampered `hash_body`) |
...
| Span unit (D14) | §12.8 declares **byte** offsets; the shipped D13 implementation emits **character** offsets — measured divergence on 4 of 5,373 pin writes, exactly the non-ASCII `rhs` cases (see §12.8's known-deviation note for the full account, including why the gate and both reviewers' five-sample checks missed it and the whole-population run did not). The fix direction is adjudicated: implementation moves to byte offsets per the declaration (the language-neutral address into the artefact `sha256`/`byte_count` describe), and the gate re-verifies on the encoded bytes, breaking its current unit-consistency with the implementation — **the gate must embody the declaration, not the code**. **Model-moving** (4 records change today, any future non-ASCII `rhs` changes with them): a D14-arc bundle item per §5.5 **Landed at D14** (`model_version` "6"): both span families now address the encoded source (one conversion at record assembly, identity on all-ASCII input), the sibling site-span defect measured and fixed in the same commit (5,111/5,113 rhs and 2,797/2,798 site spans would have been mis-addressed as bytes — the BOM at character 0), and the gates verify on encoded bytes per the row's own design rule | closed (landed, D14) |
| Descriptor invocation grammar | §3.1's descriptor carries flag and option **names** but not positional operands, option value grammar, or defaults — a round-5 reviewer enumerated the whole surface from the descriptor alone and then named this as the boundary at which "learn the interface from the tool" stops being fully true (the round-4 F4 close carried the global flags and the axes alias; this is the remaining half that close deliberately deferred). Mechanism change to the descriptor and its gate; no model interaction **Landed at D14**: each subcommand block carries `positionals` and `option_grammar` (value shape / default / choices / help), read from the parser exactly as the option list is; the gate holds declaration == parser both directions per option. §3.1's property now promises what the round measured it should: construct a valid invocation without prose | closed (landed, D14) |
| Resolved-call-site itemisation | `edges[]` carry `lines` only (§5.9); the D12 `arguments`/`span` itemisation exists solely on `unresolved_named_commands` — so a provenance chain or pre-flight terminating at a defaultless `param()` entry is sent back to the source at the **declared**-call boundary. One round-5 reviewer asked for the itemisation at resolved sites, with a §5.6-style estimate: 2,250 resolved sites (cross-checked: `sum(edges[].sites)` == `commands_named` − `unresolved_named_command_sites`) × ≈252 B/site ≈ **567 KB ≈ +52% of the default model** — and correctly noted §5.6's axis rule forbids the axis form (per-site records for resolved calls do not exist to be withheld), so this arrives as a new fact under §5.3 tiering. **Model-moving**, §5.5 bundle **Adjudicated at round 6 (prototype round, §3.2), with the axis-form reading of this row explicitly superseded.** The prototype came in at 767,914 B measured — **+35% over this row's own estimate** (the estimate priced resolved sites at the unresolved-site average; the reviewer position that costs come from emission, not estimates, is vindicated on this row's own figure). Both reviewers adjudicated: land it, land it **first** (the task the round set was completable on the trial model in one filter and impossible on the shipped model without the source). The adjudicated shape is the **`resolved-sites` axis attached to `edges[]` rows** — `site_records: [{line, span, arguments, name?}]` per edge, ordered (line, span-start), aligned with the edge's own `lines`; `name` conditional, present only when the invoked token differs from the declared name (0 at the pin — the D12 `qualifier` precedent); from/to stated once by the edge row. Measured: **491,970 B, −35.9% against the standalone collection**, default model untouched, Q3-class tasks reproduced 12/12, determinism byte-identical. Supersession grounds for this row's earlier "axis form forbidden" reading: `edges` **is** the fold — per-site detail is exactly what the per-(from,to) aggregation withheld, the same creation rule that produced `local-sites` and `command-sites`; enriching `edges[]` records keeps §5.6's collection invariant (a standalone collection as an axis would break it, and as an unconditional fact would move the default +70.5% against the round-5 refusal-is-defensible position both reviewers preserved) **Landed at D15** (`model_version` "7"): the `resolved-sites` axis ships exactly as adjudicated — `site_records` on the `edges[]` rows, conditional `name`, lines-alignment and counter ties gate-held over the whole population; measured 491,981 B on the landed build | closed (landed, D15) |
| Structured provenance layer | The other round-5 reviewer asked for a different next fact: per-`rhs` referenced-variable ids and invoked command head, plus per-use-site statically-possible supplying write-record ids — reaching-definition structure without evaluation and without a safety verdict — with the stated position that its cost must come from **a prototype's own emitted figures**, not an estimate from record counts. Recorded with that position intact. **Model-moving**, §5.5 bundle. The two round-5 asks did **not** converge (first round without convergence; §3.2): prioritisation between this row and the one above is a future adjudication, not implied by inventory order **Adjudicated at round 6 (prototype round, §3.2) — reduced, per the row's own emit-then-measure position.** The full prototype measured 3,366,658 B, and both reviewers independently rejected 79.4% of it: `supplied_by` is a precomputed reverse index — **18,501 of 18,501 lists reproduce from the model's existing `id`+`role` fields** (measured by a reviewer, re-verified) — and `sid` exists only to give it an address vocabulary; `$null` alone carried 1,093,110 B of semantically false provenance (32.5% of the increment). The adjudicated shape keeps **`rhs_refs` only** — `{variables: [{name, id}], commands: [...]}` on every rhs-bearing write, variables id-joined statically (splat `@name` joins to the variable), commands via the same §10.6 command-position walk the shipped facts use — and states the reverse join as SPEC prose at landing ("statically-possible supplying writes = every write sharing the variable id; no flow ordering, no dominance, no verdict"). **Automatic variables stay itemised** (the one point adjudicated against both reviewers' first instinct, on measurement): exclusion saves 40,877 B against **515 writes whose only rhs references are automatics, 105 of them non-constant** (`$_.Exception.Message`-class real terminals) that exclusion would collapse into literal-only rhs — and the pathology that motivated exclusion was `supplied_by`'s, which dies with it; the consumer-side filter is the `variable:automatic/` id prefix. Measured price: 692,116 B total — 26,368 B on the default (script writes; the D13 +29 KB class), 660,474 B riding the `local-sites` axis **Landed at D15** (`model_version` "7", §12.9): `rhs_refs` on every rhs-bearing write, the id joined by the site's own §12.6 classification (offset-join — one classification consumed twice; measured equal to the round-6 prototype's standalone re-tokenisation on all 5,113 pin writes before the differential gate was wired), PSS9004 id omission, automatics itemised, the reverse join as the §12.9 prose sentence; landed prices re-measured in §5.6 | closed (landed, D15) |
| Scan-outcome declaration (`PSS9001` emit path) | **The defect of §3.3, and the largest correctness gap this tool has carried.** `PSS9001` ("A region could not be parsed") is declared in §4 and compiled into `FACTS`, and has **no emit path** — measured, it occurs nowhere else in `pss.py`. The consequence is not silence but false completeness: `_match_brace` returning `None` at a `function`/`filter` declaration makes the scanner `continue` past it, so a file with one unterminated function yields a model that states the *other* functions as ordinary facts, exit 0, zero limitations. A consumer cannot distinguish "nothing there" from "could not be read", which is why the evaluating project's adoption contract begins by running a real parser first. Detection was prototyped and measured before this row was written: six named checks over the tool's **own** token stream — unterminated function body, unterminated declaration parameter list, a lexical region (quote, here-string, block comment) the tokenizer truncated at EOF, and brace / paren / bracket imbalance — detect **13 of 13** deliberately broken fixtures that native `Parser::ParseFile()` also rejects, with **zero false positives across 83 real PowerShell files (3.89 MB), the pinned basis among them, all of which the native parser confirms are error-free**. Incompleteness was then **measured** rather than left as a caveat, native `Parser::ParseFile()` acting as the oracle over 175 systematic mutations of the pinned basis itself (deletion of a closing brace / paren / bracket, of an opening brace, of a single or double quote, and truncation at a random offset — real 1.45 MB code, not hand-built fixtures): of the 147 mutations native rejects, the six checks catch **146, a detection rate of 99.3%**, with **zero false positives** across the 28 mutations native still accepts. The single miss is not a bug but a **named class**: deleting one `"` left the quote count even, so the file re-lexed with shifted string boundaries, every delimiter still balanced, and native's five errors were *grammatical* (`MissingArrayIndexExpression`, `MissingEndSquareBracket`) rather than delimiter errors. **These checks see termination and balance; a grammatical error that leaves every delimiter balanced is structurally invisible to them** — that sentence, not a percentage, is what the SPEC should carry, and the percentage is what makes it credible. Soundness measured; **completeness still not claimable** — PSS has no grammar, so the outcome it can honestly state is "no anomaly found by these named checks", never "this parses". Shape follows the evaluating analyser's always-present `Parse` block (`ErrorCount` present and zero on success, inside the stable digest) rather than a key that appears only on failure, because a key that appears only on failure cannot be distinguished from a tool that never looked. **Model-moving**, §5.5 bundle; pairs with the Emission coverage widening above | **closed — landed at the D16 arc (C1)**: SPEC 5.10 (normative), the always-present `scan` block, the six checks at the scanner's own silent-skip sites, PSS9001 emits (`check` + conditional `target`); gates: ten corrupted fixtures retained as assets, pin + cache-producer false-positive gates, the one-direction native differential (oracle version recorded per run); re-measured on the landed build: 0 anomalies over all 230 corpus generations |
| Differential ledger | Native `Parser::ParseFile()` is already a gate-time oracle here (§13.1's differential legs, the 498/491/236 degradation levels), and the §3.3 evaluation showed what a *second* implementation is worth. What is missing is the discipline for what happens when the two disagree — today a divergence is either a gate failure or nothing. The asymmetry is domain-dependent and must be stated that way: **on syntax validity native is definitionally the authority** (it is the language implementation; there is no branch where PSS is right and it is wrong), so a divergence there is a PSS defect or a PSS blind spot and nothing else — but **on derived facts it is not an authority at all**, because native returns a *tree* and every question this model answers (which call resolves where, whether a reference reads or writes, how a limitation classifies) is a choice made on top of that tree. The existing `counters.assignments is not B.4 'Assignment statements'` check is already exactly this case: a definitional divergence pinned as a difference, with neither side wrong. The row proposes recording divergences with a three-way classification — **(A)** a PSS defect, **(B)** a definitional difference, recorded and fixed in place, **(C)** a capability native has and this tool does not, which makes the ledger a **supply of §13.2 rows from the wider PowerShell community's own implementation** rather than only from survey rounds. On the oracle's version: `follow-latest`, matching the binding mode all 88 manifest units already carry, with the version and the artefact digest recorded in the gate output. That was measured before being chosen — pwsh 7.4.6 (LTS) and 7.6.5 were run over the same 16 files including the 1.45 MB basis and agreed on **every** figure (parse-error count and ids, token count, function count, CommandAst count, variable-reference count: 0 divergences), so pinning would buy little; and a version-induced divergence reddens only the `--pwsh` leg, leaving 491/236 intact, which is the buffer that makes following latest safe. A divergence appearing after an oracle update is therefore a **(C)-class ledger entry to adjudicate**, on the cold-loop pattern this repository already runs, not a CI break | **closed — landed at the D17 arc (C3)**: §13.4 states the discipline (the domain asymmetry, the (A)/(B)/(C) classes, follow-latest with per-run version recording — the recording itself landed at D16) and opens the ledger with two entries: the `counters.assignments` definitional difference (B) and the D16 ladder defect, recorded retrospectively (A) |
...
| `hash_alpha` — equality class under declared alpha-renaming | Round 8 rejected near-duplicate *similarity* (a score embeds a threshold; 3/3 reconfirmed at Round 9) and Evaluator A named the one fact-shaped alternative: a body hash with unqualified, non-automatic variables renamed in first-occurrence order — an equality partition, no score. Round 9 moved it to *proposed* (2:1, the third holding it to an exact, implemented, fixture-held rule). Measured before landing: the Round-9 text's basis ("defined on `hash_body`'s token sequence, strings already gone") is self-contradictory — `hash_body` retains strings and on that basis the pin yields **0** classes; folding strings yields the §10.3 families. **Landed at D25** (`model_version` "12") on the owner's adjudication of that basis: §10.9 — `hash_body`'s extent, `hash_full`'s string rule, then the declared rename; 471 distinct / 3 classes / 12 members at the pin held by name; `hash_body`-equal ⇒ `hash_alpha`-equal and class-invisibility-to-`hash_body`/`hash_full` gate-held; not a §10.5 rung | closed (landed, D25) |
...
### 13.4 The differential ledger (normative)

Native `Parser::ParseFile()` runs as a gate-time oracle (§13.1's
differential legs); this section is the discipline for what happens when the
two implementations **disagree** — previously a divergence was either a gate
failure or nothing. The asymmetry is domain-dependent and stated as such:
**on syntax validity native is definitionally the authority** (it is the
language implementation; there is no branch where PSS is right and it is
wrong), so a divergence there is a PSS defect or a PSS blind spot and
nothing else. **On derived facts it is not an authority at all**: native
returns a tree, and every question this model answers is a choice made on
top of one. Every recorded divergence is classified three ways:

- **(A) a PSS defect** — fixed under the standing gates, red-first, and the
  entry records the mechanism.
- **(B) a definitional difference** — neither side wrong; the entry pins the
  two definitions so the divergence stops being re-litigated.
- **(C) a capability native has and this tool does not** — a candidate
  §13.2 row: the ledger becomes a supply of improvement rows from the wider
  PowerShell implementation rather than only from survey rounds.

The oracle follows latest (`follow-latest`, the binding mode every manifest
unit carries), with the resolved version recorded in the gate output per run
(implemented at D16 — the scan differential prints it). A divergence
appearing after an oracle update is a **(C)-class entry to adjudicate** on
the repository's cold-loop pattern, not a CI break: it reddens only the
`--pwsh` leg, and the no-`pwsh`/no-`git` legs are the buffer that makes
following latest safe.

**Entries.**

| Id | Class | Divergence | Disposition |
|---|---|---|---|

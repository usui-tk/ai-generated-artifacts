...
**自己責任でご利用ください。** `psa.py` は **「現状有姿 (AS IS)」** で
提供されます。明示・黙示を問わず、 いかなる種類の保証もありません。
著作者および貢献者は、 本アナライザの使用・改変・配布から生じる損害、
検出漏れ、 正当なコードのブロック、 レビュアーの信頼の毀損、 その他
直接・間接を問わず一切の結果について責任を負いません。

`psa.py` は **発見的な静的解析器であり、 証明ツールではありません。**
本ツールの判定に依存することにより、 以下を承諾したものとみなされます:

- **false positive(偽陽性)は起こり得ます。** `psa.py` は実際には
  正しいコードを誤検出することがあります。 人によるレビューを伴わず
  に検出結果を自動マージブロッカーとして扱うと、 正当な変更の進行を
  妨げる可能性があります。 一方で、 検出内容を理解せずに無言で抑制
  することはゲートそのものの意義を毀損します。
- **false negative(偽陰性)も起こり得ます。** `psa.py` はすべての
  バグクラスを検出できるわけではありません。 後述の「検出 **しない**
  事項」セクションに対象外カテゴリの一覧があります。 また、 構文解析
  時の静的解析では原理的に捕捉できない潜在的欠陥クラスも多数存在
  します。
- **AI / LLM 利用者は、 本アナライザの出力を独立に検証しなければ
  なりません。** LLM が `psa.py` の 0/0/0 を実際のコード変更を
...
- **`psa.py` はガードレールであり、 レビューの代替ではありません。**
  ...
  LLM)レビュアーが設計をゲートします。
- **ルールの判定は変わり得ます。** 新しいルールバージョンは、 これ
  まで見えなかった規律違反を顕在化させる可能性があります。 既存ルール
...
- **`psa.py` の提案は適用前に必ずレビューしてください。** インライン
  ...
  `psa-disable-file`) はスクリプトに文面変更を加えるため、 後のレビュー
  にも耐える根拠コメントを各箇所に付与する必要があります。
...
を保持する必要があります。 本アナライザは、 上記の免責事項および
`LICENSE` ファイルに詳述されているとおり、 無保証で提供されます。
`psa.py` は Python 標準ライブラリのみを使用し、 サードパーティ依存
は import しないため、 ランタイムにサードパーティライセンスは付随
しません。

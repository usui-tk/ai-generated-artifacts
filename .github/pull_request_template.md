...
- [ ] **README sync**: if `README.md` is touched, `README.ja.md` is updated in the same PR (English is the master). Per the repository-wide documentation language policy (root `README.md` "Language Policy"), `README.md` has a Japanese counterpart and the community-health files (`CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`) are interleaved bilingual in a single file (D-CH1); `SPEC.md`, `TESTING.md`, `CHANGELOG.md`, `AGENTS.md` are English only.

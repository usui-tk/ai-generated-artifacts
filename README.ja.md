...
- バイリンガルファイルは以下の 2 パターンを許容：
  ...
  - **パターン B — プライマリ + 翻訳** (`<NAME>.md` が英語プライマリ、`<NAME>.ja.md` が日本語翻訳版)
    英語ファイル名自体が慣例として確立しているリポジトリ慣例ファイルに使用：`README.md` / `README.ja.md`、`SPEC.md` / `SPEC.ja.md`、`TESTING.md` / `TESTING.ja.md` 等。
- 単一言語ファイル: `<slug>.md`
...
- **デフォルト：日英バイリンガル** — 可能な限り、アーティファクトは英語と日本語の両方で公開（前項「ファイル」に記載した 2 パターンのいずれかを使用）
- **許容：単一言語** — バイリンガル化が現実的でない場合（言語固有の例示など）は、最も適切な言語の単一ファイルで公開
- トップレベル直下のサブディレクトリ内の README は、重複を避けるため日英を 1 ファイル内で併記してもよい（両言語が等しく完全であることが条件）。スクリプト／プロジェクト配下のディレクトリは通常、対ファイル方式（`README.md` + `README.ja.md`）を採用
